...
let count = 0;
for (let cz = 16; cz < 32; cz++) {
    for (let cx = 16; cx < 32; cx++) {
        ...
        const Y = heights[8 * 16 + 8] - 1 - (hasNegativeSections ? 64 : 0);
        ...
        if (sec && sec.block_states) {
            ...
            const index = yOffset * 256 + 8 * 16 + 8;
            name = getBlockName(palette, data, index);
        }
        console.log(`Chunk [${cx}, ${cz}] Y=${Y} block: ${name}`);
        count++;
        if (count >= 15) {
            process.exit(0);
        }
