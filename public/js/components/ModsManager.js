...
const LazyModRow = {
    ...
    template: `
        <tr :class="{'opacity-50': file.isDisabled}" ref="row" @click="$emit('click-mod', file)" class="cursor-pointer">
            <td @click.stop>
                <input class="form-check-input ms-1" type="checkbox" :value="file.name" 
            ...
            <td>
                <div class="d-flex align-items-center gap-3">
                    <div class="position-relative">
                        <img :src="iconUrl" @error="handleImgError" class="rounded border shadow-sm" width="42" height="42" style="object-fit:cover;">
                    ...
                    <div class="min-width-0">
                        <div class="fw-bold text-truncate">{{ metadata?.title || file.name }}</div>
                        <div class="d-flex gap-2 align-items-center mt-1">
                            <span v-if="metadata" class="badge bg-primary-subtle text-primary border border-primary-subtle rounded-pill small px-2">{{ metadata.version }}</span>
                            <span class="small text-muted font-monospace d-none d-md-inline">{{ file.name }}</span>
                        </div>
                    </div>
                </div>
            </td>
            <td>
                <span v-if="file.isDisabled" class="badge rounded-pill bg-warning-subtle text-warning-emphasis border border-warning-subtle">{{ $t('common.disabled') }}</span>
                <span v-else class="badge rounded-pill bg-success-subtle text-success-emphasis border border-success-subtle">{{ $t('common.enabled') }}</span>
            </td>
            <td class="text-end text-muted small pe-4">{{ (file.size/1024/1024).toFixed(2) }} MB</td>
        </tr>
    `,
    ...
    <div class="h-100 d-flex flex-column overflow-hidden">
        ...
        <template v-else>
            <div class="d-flex justify-content-between align-items-center mb-3">
                <h3>{{ $t('mods.title') }}</h3>
                <button class="btn btn-sm btn-outline-primary" @click="store.view = 'modrinth'">
            ...
            <div class="card shadow-sm d-flex flex-column" style="flex: 1; min-height: 0;">
                <div class="card-header d-flex gap-2 p-2 align-items-center bg-body-tertiary border-0 rounded-top-3">
                    ...
                    <button class="btn btn-sm btn-outline-warning border-0" @click="operateFiles('disable', selectedFiles)" :disabled="!selectedFiles.length"><i class="fa-solid fa-ban me-md-1"></i><span class="d-none d-md-inline">{{ $t('files.disable') }}</span></button>
                    <button class="btn btn-sm btn-outline-success border-0" @click="operateFiles('enable', selectedFiles)" :disabled="!selectedFiles.length"><i class="fa-solid fa-check me-md-1"></i><span class="d-none d-md-inline">{{ $t('files.enable') }}</span></button>
                    <button class="btn btn-sm btn-outline-danger border-0" @click="askDelete(selectedFiles)" :disabled="!selectedFiles.length"><i class="fa-solid fa-trash me-md-1"></i><span class="d-none d-md-inline">{{ $t('common.delete') }}</span></button>
                    ...
                    <div class="overflow-auto custom-scrollbar" style="flex: 1; min-height: 0;">
                        <table class="table table-hover align-middle mb-0">
                            <thead class="bg-body-tertiary">
                                <tr>
                                    <th style="width: 40px;"><input class="form-check-input ms-1" type="checkbox" v-model="selectAll"></th>
                                    <th>{{ $t('common.name') }}</th>
                                    <th>{{ $t('common.status') }}</th>
                                    <th class="text-end pe-4">{{ $t('common.size') }}</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr v-if="loadingList">
                                ...
                                <tr v-if="!loadingList && filteredFiles.length === 0">
                                    <td colspan="4" class="text-center py-5 text-muted">{{ $t('mods.empty') }}</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                ...
                <div v-if="selectedMod" class="modal show d-block" @click.self="selectedMod = null" style="z-index: 2070;">
                    ...
                    <div class="modal-content shadow-lg border-0 rounded-4 overflow-hidden bg-body h-100" style="max-height: 90vh;">
                        ...
                        <div class="modal-body p-0 overflow-hidden d-flex flex-column flex-md-row">
                            ...
                            <div class="col-12 overflow-auto p-4 custom-scrollbar bg-body">
                                <div class="d-flex gap-4 mb-4" v-if="selectedMod.project">
                                ...
                                <div v-else class="text-center py-4 bg-body-tertiary rounded-4 mb-4">
                                    <i class="fa-solid fa-file-zipper fa-3x mb-3 opacity-25"></i>
                                    <h5 class="fw-bold">{{ selectedMod.file.name }}</h5>
                                ...
                                <div v-if="selectedMod.project" class="border-top pt-4">
                                    <div class="d-flex justify-content-between align-items-center mb-3">
                                        ...
                                        <button class="btn btn-xs btn-outline-primary rounded-pill px-3 fw-bold" @click="translateBody" :disabled="translatingBody">
