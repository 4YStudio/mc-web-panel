

export const messages = {
    zh: {
        common: {
            cancel: '取消',
            confirm: '确认',
            execute: '执行',
            save: '保存',
            danger_zone: '危险区域',
            delete: '删除',
            edit: '编辑',
            rename: '重命名',
            search: '搜索',
            filter: '筛选',
            back: '返回',
            loading: '加载中...',
            load_more: '加载更多',
            success: '操作成功',
            error: '操作失败',
            delete_confirm: '确定要删除 {count} 项吗？',
            yes: '是',
            no: '否',
            enabled: '已启用',
            disabled: '已禁用',
            unknown: '未知',
            actions: '操作',
            name: '名称',
            size: '大小',
            time: '时间',
            status: '状态',
            switchTheme: '切换主题',
            switchLang: 'Switch Language',
            logout: '退出登录',
            upload: '上传',
            download: '下载',
            refresh: '刷新',
            close: '关闭',
            add: '添加',
            remove: '移除',
            mode_gui: '图形模式',
            mode_text: '文本模式',
            gui_mode: '图形模式',
            text_mode: '文本模式',
            player: '玩家',
            player_name: '玩家名称',
            action_sent: '操作已发送',
            select_fail: '选择失败',
            save_fail: '保存失败',
            server_offline: '服务器已离线',
            note: '备注',
            lock: '锁定',
            unlock: '解锁',
            locked: '已锁定',
            edit_note: '编辑备注',
            processing: '处理中...',
            no_data: '暂无数据',
            reset: '重置',
            install: '安装',
            installing: '正在安装...',
            dont_show_again: '不再提示',
            unsaved_changes: '您有未保存的更改，确定要离开吗？',
            export: '导出'
        },
        cancel: '取消',
        close: '关闭',
        login: {
            title: 'MC 面板',
            prompt_scan: '请使用 Google Authenticator 扫描此二维码。',
            placeholder_code: '请输入 2FA 验证码',
            btn_verify: '验证身份',
            toast_fail: '验证码错误',
            toast_error: '请求失败',
            placeholder_user: '请输入用户名',
            placeholder_pass: '请输入密码',
            placeholder_captcha: '验证码',
            slide_to_verify: '安全验证',
            click_to_verify: '点击完成验证',
            verified_success: '已通过安全验证',
            captcha_modal_title: '请完成安全验证',
            drag_tip: '向右滑动滑块完成拼图',
            refresh_captcha: '换一张',
            verify_failed: '拼图未对齐，请重新滑动',
            loading_captcha: '正在加载验证码...',
            placeholder_confirm_pass: '请确认新密码',
            btn_login: '登 录',
            btn_next: '下一步',
            btn_prev: '返回上一步',
            enable_2fa: '开启 2FA 双重认证（推荐）',
            init_title: '初始化管理员账号',
            btn_init: '完成初始化',
            toast_init_success: '账号初始化成功',
            toast_init_fail: '初始化失败',
            toast_login_fail: '用户名或密码错误',
            toast_captcha_fail: '验证码错误',
            locked_prefix: '登录失败次数过多，请等待 ',
            locked_suffix: ' 秒后再试',
            mismatch_pass: '两次输入的密码不一致',
            switch_to_2fa: '使用 2FA 令牌直登',
            switch_to_password: '使用账号密码登录',
            disaster_recovery: '灾难恢复 / 从备份还原',
            recovery_title: '系统灾难恢复',
            recovery_desc: '若面板配置损坏或无法正常登录，可输入管理员密码并上传系统全局备份压缩包直接覆盖还原。',
            recovery_pass_label: '管理员登录密码',
            recovery_pass_placeholder: '请输入管理员登录密码',
            recovery_file_label: '备份压缩包文件 (.zip)',
            recovery_select_file: '点击选择备份压缩包 (.zip)',
            recovery_btn_start: '验证密码并开始回档',
            recovery_btn_cancel: '返回登录',
            recovery_no_pass: '请输入管理员密码以授权回档',
            recovery_no_file: '请先选择要还原的备份压缩包'
        },
        sidebar: {
            title: 'MC 面板',
            dashboard: '控制台',
            settings: '服务器设置',
            mods: '模组管理',
            files: '文件管理',
            backups: '地图备份',
            auth: '认证管理',
            voicechat: '语音设置',
            players: '玩家管理',
            modrinth: '模组下载',
            java: 'Java 管理',
            about: '关于',
            plugins: '插件管理',
            frp_manager: 'FRP 内网穿透',
            scrolls: '卷轴工坊',
            customize: '自定义侧边栏',
            customize_desc: '调整侧边栏项目的显示和顺序。隐藏的项目仍然可以通过其他方式访问。',
            reset: '恢复默认'
        },
        scrolls: {
            title: '卷轴管理',
            description: '管理和配置自动化脚本卷轴，通过控制台事件与指令注入实现游戏自动化',
            guide_button: '开发文档',
            market_button: '官方商店',
            install: '安装卷轴',
            market_link: '前往卷轴市场',
            market_hint: '想要发现更多免侵入自动化玩法？欢迎访问官方卷轴市场。',
            market_title: '发现更多官方与社区卷轴？',
            market_desc: '访问 GitHub 托管的官方卷轴市场，免费浏览下载扫地姬、防沉迷、快速天亮等丰富脚本。',
            official_market: '官方卷轴市场',
            disclaimer_title: '卷轴安全免责声明',
            disclaimer_line1: '卷轴系统基于 Minecraft 服务端控制台命令与事件驱动执行，具备向服务器下发控制台指令、广播消息及管理实例存储等能力。',
            disclaimer_line2: '安装第三方卷轴前，请务必确认脚本来源可信。因使用未知或恶意卷轴导致的服务器崩溃、存档异常或数据丢失，本面板概不承担责任。',
            disclaimer_line3: '使用本卷轴系统即表示您已了解并同意上述风险提示，所有因卷轴脚本运行产生的后果由用户自行承担。',
            disclaimer_confirm: '我已阅读并理解上述风险提示与免责声明',
            disclaimer_agree: '确认并继续',
            upload_desc: '请上传 .zip 格式的卷轴包。系统将对卷轴包进行结构与元数据解析。',
            install_file_label: '选择卷轴 ZIP 文件',
            install_confirm_title: '确认安装卷轴',
            update_title: '更新卷轴: {name}',
            new_badge: '新卷轴',
            update_badge: '更新',
            update_comparison: '版本对比',
            current_version: '当前版本',
            new_version: '准备安装',
            downgrade_warning: '注意：准备安装的版本低于当前已安装版本。',
            install_confirm_hint: '请确认以上卷轴信息，确认无误后点击下方按钮开始部署安装。',
            confirm_update: '确认更新',
            confirm_install: '确认安装',
            upload_analyze: '上传并解析',
            analyzing: '正在解析...',
            installed_success_toast: '卷轴 {name} 安装成功！',
            config_schema_title: '可配置参数',
            running: '运行中',
            stopped: '已停止',
            error: '加载错误',
            no_scrolls: '暂无已安装卷轴',
            no_scrolls_hint: '可以从卷轴市场下载 ZIP 压缩包或自制脚本后上传安装',
            settings: '配置',
            save_config: '保存配置',
            config_saved: '配置已保存并即时热生效！',
            export: '导出',
            delete: '删除',
            delete_title: '卸载卷轴',
            delete_confirm: '确定要彻底删除卷轴 {name} 吗？',
            delete_warning_hint: '删除卷轴将永久移除该卷轴的所有脚本文件与运行数据。',
            deleted_success: '{name} 已卸载'
        },
        about: {
            title: '关于 MWP',
            description: '一个轻量级、现代化的 Fabric 服务器管理面板。',
            version: '版本',
            author: '作者',
            version: '版本',
            author: '作者',
            source: '源代码',
            website: '官方网站',
            check_update: '检查更新',
            checking: '检查中...',
            update_available: '发现新版本！',
            latest_version: '已是最新版本',
            new_version: '最新版本',
            download: '前往下载',
            update_now: '立即更新',
            updating: '正在更新...',
            current_is_latest: '您当前使用的是最新版本。',
            released_at: '发布时间',
            show_changelog: '查看更新日志',
            hide_changelog: '收起更新日志'
        },
        dashboard: {
            console_title: '控制台',
            start: '启动服务器',
            stop: '停止服务器',
            force_stop: '强制关闭',
            force_stop_confirm_title: '强制关闭服务器',
            force_stop_confirm_msg: '确定要强制关闭服务器吗？<br><strong class="text-danger">此操作将立即终止服务器进程，可能导致数据丢失！</strong><br>请仅在服务器无法正常关闭时使用此功能。',
            force_stop_sent: '已发送强制关闭指令',
            state_starting: '启动中',
            state_running: '运行中',
            state_stopping: '停止中',
            state_stopped: '已停止',
            server_info: 'MC 服务器信息',
            online_players: '在线人数',
            system_resource: '系统资源',
            cpu_usage: 'CPU 使用率',
            ram_usage: '内存使用率',
            mem_usage: '内存',
            swap_usage: '交换分区',
            disk_usage: '磁盘存储',
            swap_unconfigured: '未配置',
            status: '状态',
            status_normal: '正常',
            status_high: '高负荷',
            send_cmd_placeholder: '发送指令...',
            send: '发送',
            toast_sent: '已发送',
            system: '系统',
            no_players: '暂无在线',
            target: '游戏版本',
            loader: '核心架构',
            java_version: 'Java 环境',
            port: '服务端口',
            collapse_info: '收起',
            expand_info: '展开',
            cmd_helper: '指令助手',
            cmd_all: '全部',
            cmd_cat_player: '玩家',
            cmd_cat_server: '服务器',
            cmd_cat_world: '世界',
            cmd_cat_admin: '管理',
            cmd_cat_advanced: '高级',
            cmd_search: '搜索指令...',
            cmd_quick: '快捷指令',
            cmd_quick_title: '快捷发送',
            cmd_no_result: '未找到匹配的指令',
            cmd_advancement_desc: '授予或撤销玩家进度',
            cmd_attribute_desc: '查询或修改实体属性',
            cmd_clear_desc: '清空玩家背包物品',
            cmd_clone_desc: '复制区域内的方块',
            cmd_damage_desc: '对实体造成伤害',
            cmd_data_desc: '获取/修改方块或实体NBT数据',
            cmd_datapack_desc: '管理数据包',
            cmd_debug_desc: '调试性能分析',
            cmd_defaultgamemode_desc: '设置默认游戏模式',
            cmd_difficulty_desc: '设置游戏难度',
            cmd_effect_desc: '给予或清除玩家药水效果',
            cmd_enchant_desc: '附魔玩家手持物品',
            cmd_experience_desc: '添加/设置/查询玩家经验',
            cmd_fill_desc: '用指定方块填充区域',
            cmd_fillbiome_desc: '用指定生物群系填充区域',
            cmd_forceload_desc: '强制加载/卸载区块',
            cmd_function_desc: '运行函数',
            cmd_gamemode_desc: '设置玩家游戏模式',
            cmd_gamerule_desc: '查询或修改游戏规则',
            cmd_give_desc: '给予玩家物品',
            cmd_help_desc: '显示帮助信息',
            cmd_item_desc: '修改/替换物品栏物品',
            cmd_kick_desc: '将玩家踢出服务器',
            cmd_kill_desc: '杀死实体',
            cmd_list_desc: '列出在线玩家',
            cmd_locate_desc: '定位最近的结构/生物群系',
            cmd_loot_desc: '给予战利品表物品',
            cmd_msg_desc: '向玩家发送私聊消息',
            cmd_particle_desc: '创建粒子效果',
            cmd_place_desc: '放置结构/地物',
            cmd_playsound_desc: '播放音效',
            cmd_random_desc: '随机数生成',
            cmd_reload_desc: '重新加载战利品表/函数等',
            cmd_recipe_desc: '给予/移除玩家配方',
            cmd_ride_desc: '让实体骑乘/离开',
            cmd_rotate_desc: '旋转实体朝向',
            cmd_say_desc: '向所有玩家发送消息',
            cmd_schedule_desc: '延时执行函数',
            cmd_scoreboard_desc: '管理计分板',
            cmd_seed_desc: '显示世界种子',
            cmd_setblock_desc: '在指定位置放置方块',
            cmd_setworldspawn_desc: '设置世界出生点',
            cmd_spawnpoint_desc: '设置玩家出生点',
            cmd_spectate_desc: '旁观指定玩家',
            cmd_spreadplayers_desc: '将玩家分散到区域',
            cmd_stopsound_desc: '停止播放音效',
            cmd_summon_desc: '召唤实体',
            cmd_tag_desc: '管理实体标签',
            cmd_team_desc: '管理队伍',
            cmd_teammsg_desc: '向队伍发送消息',
            cmd_teleport_desc: '传送实体',
            cmd_tellraw_desc: '向玩家发送JSON消息',
            cmd_time_desc: '查询/设置游戏时间',
            cmd_title_desc: '向玩家显示标题',
            cmd_trigger_desc: '修改触发器计分板',
            cmd_weather_desc: '设置天气',
            cmd_worldborder_desc: '管理世界边界',
            cmd_ban_desc: '封禁玩家',
            cmd_ban_ip_desc: '封禁IP地址',
            cmd_banlist_desc: '查看封禁列表',
            cmd_deop_desc: '撤销管理员权限',
            cmd_op_desc: '给予管理员权限',
            cmd_pardon_desc: '解封玩家',
            cmd_pardon_ip_desc: '解封IP地址',
            cmd_save_all_desc: '保存世界存档',
            cmd_save_off_desc: '关闭自动保存',
            cmd_save_on_desc: '开启自动保存',
            cmd_setidletimeout_desc: '设置空闲踢出时间',
            cmd_stop_desc: '关闭服务器',
            cmd_whitelist_desc: '管理白名单',
            cmd_execute_desc: '条件执行命令',
            cmd_bossbar_desc: '管理Boss栏',
            cmd_tick_desc: '查询/修改游戏刻速率',
            cmd_tp_desc: '快速传送实体到目标或坐标',
            cmd_tell_desc: '向玩家发送私聊消息',
            cmd_w_desc: '私聊密语发送消息',
            cmd_xp_desc: '增减或设置玩家经验等级',
            cmd_me_desc: '发送动作叙述广播消息',
            cmd_perf_desc: '服务端专用性能分析采样',
            cmd_jfr_desc: 'Java飞行记录仪性能分析',
            cmd_publish_desc: '开放为局域网联机模式',
            cmd_return_desc: '从函数控制流返回指定数值'
        },
        files: {
            path: '路径',
            operate: '批量操作',
            new_folder: '新建文件夹',
            new_file: '新建文件',
            go_up: '返回上一级',
            upload_file: '上传文件',
            upload_folder: '上传文件夹',
            upload_tips: '拖拽文件到这里上传',
            total: '共 {count} 项',
            move: '移动',
            copy: '复制',
            compress: '压缩',
            extract: '解压',
            drop_to_upload: '拖放文件到此处上传',
            disable: '禁用',
            enable: '启用',
            modal_move_title: '移动/复制文件',
            modal_move_dest: '目标路径 (相对于根目录)',
            modal_compress_title: '压缩文件',
            modal_compress_name: '压缩包名称 (.zip)',
            modal_new_folder: '文件夹名称',
            modal_new_file: '文件名称',
            modal_rename: '新名称',
            modal_edit_warning_title: '警告',
            modal_edit_warning_msg: '文件 {name} 可能不是文本格式。确定要编辑吗？',
            file_not_found: '文件 {name} 未找到，请检查文件是否存在',
            folder_not_found: '文件夹 {name} 未找到，请检查文件夹是否存在',
            clipboard_msg: '已{action} {count} 个文件',
            extract_to: '解压到',
            extract_here: '解压到当前目录',
            extract_to_folder: '解压到指定目录',
            modal_extract_title: '解压文件',
            modal_extract_dest: '目标目录 (留空则解压到同名文件夹)',
            preview_image: '预览图片',
            preview_archive: '预览压缩包',
            compressing: '正在压缩...',
            extracting: '正在解压...',
            upload_confirm_title: '确认上传文件',
            upload_confirm_desc: '请确认并选择需要上传的文件列表（已自动选中所有项目）：',
            upload_list_title: '待上传文件列表',
            error_read: '读取文件失败',
            editor: {
                unsaved: '未保存',
                saved: '已保存',
                search: '搜索',
                search_tip: '查找与替换 (Ctrl+F / Ctrl+H)',
                search_placeholder: '查找内容...',
                replace: '替换',
                replace_all: '全部替换',
                replace_placeholder: '替换为...',
                find_prev: '上一个',
                find_next: '下一个',
                no_match: '无匹配项',
                replace_count: '已成功替换 {count} 处匹配项',
                format: '格式化',
                format_tip: '美化格式 (JSON/代码)',
                format_success: '代码格式化完成',
                already_formatted: '当前内容格式已非常规范，无需调整',
                format_json_fail: 'JSON 语法错误，无法格式化',
                goto_line: '跳转',
                goto_tip: '跳转到指定行',
                goto_prompt: '请输入要跳转的行号 (1 - {max}):',
                wrap_on: '自动换行',
                wrap_off: '不换行',
                wrap_tip: '切换自动折行',
                font_size_tip: '切换字体大小',
                fullscreen: '全屏模式',
                exit_fullscreen: '退出全屏',
                lines: '行数',
                length: '字符数',
                modified: '已修改'
            }
        },
        mods: {
            title: '模组列表',
            source_local: '本地解析',
            file_info: '模组与文件详情',
            filename: '文件名称',
            filesize: '文件大小',
            no_desc: '该模组暂未提供描述信息',
            upload_mod: '上传模组',
            upload_placeholder: '点击或拖拽上传 .jar 文件',
            search_placeholder: '搜索模组...',
            version: '版本',
            empty: '暂无模组',
            browse_modrinth: '浏览 Modrinth',
            search_modrinth: '搜索 Modrinth 模组...',
            install: '安装',
            installing: '正在安装...',
            download_success: '模组 {name} 下载成功',
            download_fail: '下载失败: {error}',
            select_version: '选择版本',
            no_versions: '未找到兼容的版本',
            project_info: '基本信息',
            loader: '加载器',
            game_version: '游戏版本',
            description: '描述',
            categories_title: '分类',
            categories: {
                adventure: '冒险',
                cursed: '搞怪',
                decoration: '装饰',
                economy: '经济',
                equipment: '装备',
                food: '食物',
                library: '常用库',
                magic: '魔法',
                optimization: '优化',
                storage: '存储',
                utility: '工具',
                worldgen: '世界生成',
                technology: '技术',
                fabric: 'Fabric',
                forge: 'Forge',
                quilt: 'Quilt',
                neoforge: 'NeoForge',
                canvas: 'Canvas',
                social: '社交',
                transportation: '交通',
                management: '管理',
                'game-mechanics': '游戏机制',
                minigame: '小游戏',
                mobs: '生物'
            },
            modrinth: {
                project_info: '基本信息',
                introduction: '模组详细介绍',
                found_versions: '找到 {count} 个版本',
                project_type: '项目类型',
                environment: '运行环境',
                env_server: '服务端',
                env_client: '客户端',
                sort: '排序方式',
                types: {
                    mod: '模组 (Mod)',
                    plugin: '插件 (Plugin)',
                    datapack: '数据包 (Datapack)',
                    resourcepack: '资源包'
                },
                env: {
                    server: '服务器端支持',
                    client: '客户端必须'
                },
                sorting: {
                    relevance: '相关度',
                    downloads: '下载量',
                    follows: '关注数',
                    newest: '最新发布',
                    updated: '最近更新'
                },
                last_updated: '更新于',
                env_required: '必须',
                env_optional: '可选',
                env_unsupported: '不支持',
                searching: '正在搜索 Modrinth...',
                no_results: '未找到结果',
                reset_filters: '重置所有过滤器',
                try_different: '尝试不同的过滤器或更广泛的搜索词'
            },
            translate: 'AI 翻译',
            translating: '正在翻译...',
            translate_fail: 'AI 配置无效或翻译失败',
            translate_fail_config: 'AI 配置无效, 请在面板设置中检查',
            upload_file: '上传文件',
            upload_folder: '上传文件夹',
            upload_confirm_title: '确认上传模组文件',
            upload_confirm_desc: '检测到您上传的文件包含文件夹或非 .jar 的文件，请在下方确认需要上传的文件：',
            compliant_files: '合规文件 (.jar)',
            no_compliant: '无合规文件',
            non_compliant_files: '不合规文件',
            no_non_compliant: '无不合规文件',
            is_folder: '文件夹',
            is_non_jar: '非 JAR',
            conflict_btn: '冲突模组',
            conflict_title: '解决模组版本冲突',
            conflict_list: '冲突模组列表 (共 {count} 个)',
            versions_count: '{count} 个版本',
            earliest_time: '最早: {time}',
            conflict_desc: '请选择一个您想保留的版本，系统将自动删除其它版本文件以恢复正常加载。',
            file_size: '文件大小: {size}',
            mtime: '修改时间: {time}',
            metadata_version: '模组版本: {version}',
            keep_this_version: '保留此版本',
            no_conflicts: '当前没有检测到任何版本冲突的模组',
            skip_confirm: '选择版本后不要弹出确认提示框',
            confirm_keep_title: '确认保留版本',
            confirm_keep_msg: '确定要保留模组版本 [{name}] 并删除其他 {count} 个旧/冲突版本吗？',
            no_info: '该模组暂无更多详细说明'
        },

        players: {
            title: '玩家管理',
            online_players: '在线玩家',
            no_online: '当前没有玩家在线',
            empty: '暂无玩家',
            search_player: '搜索玩家...',
            player_name: '玩家名称',
            whitelist: '白名单',
            ops: '管理员 (OP)',
            bans: '黑名单',
            kick: '踢出',
            ban: '封禁',
            teleport: '传送玩家',
            gamemode: '修改模式',
            clear_inv: '清空背包',
            kill: '击杀玩家',
            reason: '原因',
            expires: '到期',
            permanent: '永久',
            more_actions: '更多操作',
            give_item: '给予物品',
            give_item_prompt: '物品ID 数量 (如: minecraft:diamond 1)',
            search_item_placeholder: '搜索物品名称或 ID...',
            selected_item: '选中的物品',
            no_selection: '请选择物品',
            custom_item_id_label: '手动输入自定义物品 ID',
            quantity_label: '数量',
            qty_unit: '个',
            tab_building_blocks: '建筑',
            tab_decorations: '装饰',
            tab_redstone: '红石',
            tab_transportation: '交通运输',
            tab_tools: '工具',
            tab_combat: '战斗',
            tab_food: '食品',
            tab_materials: '材料',
            tab_brewing: '酿造',
            tab_misc: '杂项',
            effect: '给予效果',
            effect_prompt: '效果ID 持续时间(秒) 等级 (如: minecraft:speed 600 1)',
            experience: '给予经验',
            experience_prompt: '经验值 (如: 100)',
            title_action: '发送标题',
            title_prompt: '标题内容',
            message: '私聊消息',
            message_prompt: '消息内容',
            spawnpoint: '重置出生点',
            whitelist_add: '添加白名单',
            op_give: '给予OP',
            ban_ip: '封禁IP',
            ban_reason_prompt: '封禁原因（可选）',
            ban_reason_placeholder: '输入封禁原因...',
            effect_type: '效果类型',
            effect_duration: '持续时间',
            seconds: '秒',
            effect_level: '效果等级',
            level_unit: '级',
            hide_particles: '隐藏粒子效果',
            clear_effects: '清除所有效果',
            inventory: '查看背包',
            total_items: '物品总数',
            unique_items: '不同物品',
            enchanted: '附魔',
            op_remove: '移除OP',
            armor: '装备',
            offhand: '副手',
            hotbar: '快捷栏'
        },
        easyauth: {
            users: '用户管理',
            registered: '已注册',
            unregistered: '未注册',
            password: '修改密码',
            unregister: '注销/删除',
            confirm_unregister: '确定要注销玩家',
            config: '配置',
            new_password_prompt: '输入新密码 (>4 字符):',
            new_password_placeholder: '新密码...',
            config_groups: {
                login_session: '登录会话设置',
                global_password: '全局密码设置',
                others: '其他设置',
                restrictions: '限制设置',
                rules: '规则设置',
                storage: '存储设置',
                technical: '技术设置'
            },
            config_labels: {
                premium_auto_login: '正版自动登录',
                offline_by_default: '默认离线模式',
                session_timeout: '会话超时 (秒)',
                max_login_tries: '最大登录尝试次数',
                kick_timeout: '踢出超时 (秒)',
                enable_global_password: '启用全局密码',
                single_use_global_password: '一次性全局密码',
                hide_player_coords: '隐藏玩家坐标',
                debug: '调试模式',
                allow_chat: '允许聊天',
                allow_commands: '允许指令',
                allow_movement: '允许移动',
                allow_block_interaction: '允许方块交互',
                allow_block_breaking: '允许破坏方块',
                hide_inventory: '隐藏物品栏',
                player_invulnerable: '玩家无敌',
                min_password_length: '最小密码长度',
                max_password_length: '最大密码长度',
                username_regexp: '用户名正则',
                enable_server_side_translation: '启用服务端翻译',
                database_type: '数据库类型',
                sqlite_path: 'SQLite 路径',
                sqlite_table: 'SQLite 表名',
                mysql_host: 'MySQL 主机',
                mysql_user: 'MySQL 用户',
                mysql_password: 'MySQL 密码',
                mysql_database: 'MySQL 数据库',
                mysql_table: 'MySQL 表名',
                mongodb_connection_string: 'MongoDB 连接字符串',
                mongodb_database: 'MongoDB 数据库',
                use_simpleauth_db: '使用 SimpleAuth 数据库',
                global_password_hash: '全局密码哈希'
            }
        },
        voice: {
            title: '简单语音配置 (Simple Voice Chat)',
            general: '基础与网络',
            network: '网络与连接',
            port: '端口',
            port_desc: '语音传输 UDP 端口 (默认 24454)',
            host: '语音主机 (voice_host)',
            host_desc: '客户端连接的公网 IP 或域名加端口，如 mc.example.com:24454',
            bind_address: '绑定地址 (bind_address)',
            bind_address_desc: '服务器绑定的本地网卡 IP，留空或填 * 绑定所有地址',
            password: '连接密码 (向下兼容)',
            saved: '配置已保存 (需重启服务器生效)',
            restart_required: '修改配置后需要重启服务器才能生效。',
            quality_distance: '语音范围与音质',
            max_distance: '最大语音距离',
            max_distance_desc: '普通玩家语音可听到的最大半径 (默认 48.0)',
            whisper_distance: '耳语距离',
            whisper_distance_desc: '轻声耳语可听到的最大距离 (默认 24.0)',
            broadcast_range: '广播范围',
            broadcast_range_desc: '语音广播的范围，设为 -1 代表跟随最大语音距离',
            codec: '编码器模式 (codec)',
            codec_desc: 'Opus 编码器模式：VOIP (语音通话优先)、AUDIO (高音质优先) 或 RESTRICTED_LOWDELAY (超低延迟)',
            mtu_size: '音频包大小 (mtu_size)',
            mtu_size_desc: '单音频包最大字节数 (默认 1024，不建议调小)',
            crouch_distance: '潜行距离 (向下兼容)',
            bitrate: '比特率 (向下兼容)',
            max_packet_weight: '最大包权重 (向下兼容)',
            groups_permissions: '群组与互动权限',
            enable_groups: '启用群组通话',
            enable_groups_desc: '是否允许玩家创建和加入私密语音群组',
            allow_recording: '允许录制语音',
            allow_recording_desc: '是否允许玩家在客户端录制语音对话',
            spectator_interaction: '旁观者通话',
            spectator_interaction_desc: '是否允许处于旁观者模式的玩家与普通存活玩家通话',
            spectator_player_possession: '附身通话',
            spectator_player_possession_desc: '旁观者附身在玩家身上时是否允许通话',
            security_performance: '安全与性能设置',
            force_voice_chat: '强制安装模组',
            force_voice_chat_desc: '强制所有进服玩家必须安装语音聊天模组，否则将被踢出',
            login_timeout: '模组检测超时 (毫秒)',
            login_timeout_desc: '玩家进入服务器等待检查语音模组的超时时间 (默认 10000)',
            tcp_rate_limit: 'TCP 速率限制',
            tcp_rate_limit_desc: '玩家每秒最多发送的 TCP 语音控制包数 (默认 16，-1 禁用)',
            keep_alive: '心跳包间隔 (毫秒)',
            keep_alive_desc: '心跳维持数据包的发送频率 (默认 1000)',
            allow_pings: '允许外部 Ping',
            allow_pings_desc: '语音聊天服务端是否响应外部状态检测 Ping',
            use_natives: '加载原生动态库',
            use_natives_desc: '在专用服务器上加载原生加速动态库 (插件和附属模组支持)',
            threaded_server_support: '多线程服务器支持',
            threaded_server_support_desc: '支持在多线程服务端 (如 Folia) 下运行语音聊天',
            template_notice: '已自动提供默认配置模板，点击保存即可写入实例配置文件。',
            open_anyway: '强制打开配置'
        },
        properties: {
            title: '服务器设置 (server.properties)',
            server_icon: '服务器图标',
            reset_icon_confirm: '确定要重置图标与其删除吗？',
            icon_tips: '支持任意图片（将自动裁剪并转换为 64x64 PNG）',
            icon_auto_converted: '图片已自动裁剪并转换为 64x64 PNG 格式',
            port_conflict_title: '端口冲突',
            port_conflict_msg: '检测到当前端口 [{port}] 已被占用，服务器无法启动。是否一键将端口自动重分配为可用端口并直接启动？',
            port_reassign_success: '端口已成功自动修改为 {port}，正在启动服务器...',
            filter: '筛选设置...',
            show_advanced: '高级设置 (Expert)',
            restart_tips: '修改后需要重启服务器才能生效。',
            key: '键',
            value: '值',
            groups: {
                general: '基础设置',
                gameplay: '游戏规则',
                world: '世界生成',
                spawning: '生成控制',
                network: '性能与网络',
                security: '安全与远程',
                resource_pack: '资源包',
                permissions: '权限控制',
                management: '管理服务',
                other: '其他设置'
            },
            labels: {
                motd: '服务器描述',
                'server-port': '端口',
                'max-players': '最大玩家数',
                'online-mode': '在线模式',
                'white-list': '白名单',
                'enable-rcon': '启用 RCON',
                'rcon.password': 'RCON 密码',
                'rcon.port': 'RCON 端口',
                gamemode: '游戏模式',
                'force-gamemode': '强制游戏模式',
                difficulty: '难度',
                hardcore: '极限模式',
                pvp: 'PVP',
                'allow-flight': '允许飞行',
                'level-seed': '种子',
                'level-type': '世界类型',
                'level-name': '世界名称',
                'generate-structures': '生成结构',
                'generator-settings': '生成器设置',
                'allow-nether': '允许下界',
                'spawn-monsters': '生成怪物',
                'spawn-animals': '生成动物',
                'spawn-npcs': '生成村民',
                'spawn-protection': '出生点保护',
                'view-distance': '视距',
                'simulation-distance': '模拟距离',
                'max-tick-time': '最大Tick时间',
                'rate-limit': '速率限制',
                'enable-query': '启用 Query',
                'query.port': 'Query 端口',
                'enable-status': '启用状态',
                'enforce-secure-profile': '强制安全档案',
                'enforce-whitelist': '强制白名单',
                'entity-broadcast-range-percentage': '实体广播范围',
                'function-permission-level': '函数权限等级',
                'op-permission-level': 'OP 权限等级',
                'player-idle-timeout': '玩家空闲超时',
                'prevent-proxy-connections': '阻止代理连接',
                'network-compression-threshold': '网络压缩阈值',
                'max-world-size': '最大世界大小',
                'sync-chunk-writes': '同步区块写入',
                'use-native-transport': '使用原生传输',
                'enable-jmx-monitoring': '启用 JMX 监控',
                'broadcast-console-to-ops': '控制台广播至OP',
                'broadcast-rcon-to-ops': 'RCON 广播至OP',
                'require-resource-pack': '强制资源包',
                'resource-pack': '资源包 URL',
                'resource-pack-sha1': '资源包 SHA1',
                'resource-pack-prompt': '资源包提示',
                'resource-pack-id': '资源包 ID',
                'server-ip': '服务器 IP',
                'hide-online-players': '隐藏在线玩家',
                'initial-enabled-packs': '初始启用的包',
                'initial-disabled-packs': '初始禁用的包',
                'max-chained-neighbor-updates': '最大链式更新',
                'log-ips': '记录 IP',
                'region-file-compression': '区域文件压缩',
                'pause-when-empty-seconds': '无人暂停秒数',
                'accepts-transfers': '接受转移',
                'enable-code-of-conduct': '启用行为准则',
                'bug-report-link': 'Bug 报告链接',
                'text-filtering-config': '文本过滤配置',
                'text-filtering-version': '文本过滤版本',
                'management-server-enabled': '启用管理服务',
                'management-server-host': '管理服务主机',
                'management-server-port': '管理服务端口',
                'management-server-secret': '管理服务密钥',
                'management-server-allowed-origins': '管理服务允许来源',
                'management-server-tls-enabled': '启用 TLS',
                'management-server-tls-keystore': 'TLS 密钥库',
                'management-server-tls-keystore-password': 'TLS 密钥库密码',
                'status-heartbeat-interval': '状态心跳间隔',
                'enable-command-block': '启用命令方块'
            },
            fabric_version: 'Fabric 核心版本',
            loader_version: '加载器核心版本',
            loader_type: '加载器类型',
            current_mc: '当前 MC 版本',
            current_loader: '当前 Loader 版本',
            change_version: '更换版本',
            target_mc: '目标 MC 版本',
            target_loader: '目标 Loader 版本',
            choose_mc: '选择 MC 版本',
            choose_loader: '选择 Loader 版本',
            apply: '应用',
            stop_server_first: '请先停止服务器再更换版本',
            version_change_tips: '更换核心版本不会影响地图和配置文件，但可能与部分 Mod 不兼容。建议更换后检查 Mod 兼容性。',
            fetch_versions_fail: '获取版本列表失败',
            change_version_confirm: '确定要将加载器核心更换为 MC {mc} / Loader {loader} 吗？',
            version_changing: '正在下载并更换加载器核心...',
            version_change_success: '加载器核心版本更换成功',
            version_change_timeout: '版本更换超时，请检查服务器日志确认状态'
        },
        panel_settings: {
            title: '面板设置',
            basic: '基础设置',
            server: '服务器配置',
            security: '安全设置',
            integrations: '集成与回调',
            advanced: '高级设置',
            port: '面板端口',
            port_desc: '修改后需要重启面板',
            host: '监听地址 (Host)',
            host_desc: '面板监听的网络接口。0.0.0.0 允许所有网卡/远程访问，127.0.0.1 仅允许本机访问',
            host_all: '0.0.0.0 (所有接口/允许远程)',
            host_local: '127.0.0.1 (仅本机访问)',
            lan_addresses: '检测到的局域网访问地址',
            default_lang: '默认语言',
            theme: '主题模式',
            theme_light: '浅色',
            theme_dark: '深色',
            theme_auto: '跟随系统',
            jar_name: '服务器启动文件',
            jar_name_desc: '选择用于启动服务器的 JAR 文件',
            java_args: 'Java参数',
            java_args_desc: '每行一个参数,如 -Xms1G',
            secret: '2FA密钥',
            secret_masked: '密钥已隐藏',
            session_timeout: '会话超时(天)',
            max_log_history: '日志历史条数',
            monitor_interval: '监控刷新间隔(ms)',
            reset_2fa: '重置2FA',
            restart_panel: '重启面板',
            restart_now: '立即重启',
            restart_later: '暂不重启',
            save_success: '配置已保存',
            restart_required: '需要重启面板才能生效',
            restart_confirm: '配置已保存,需要重启面板才能生效。是否立即重启?',
            reset_2fa_confirm: '确定要重置2FA吗?这将使所有已登录的设备失效,您需要重新扫描二维码设置。',
            reset_2fa_setup: '设置新 2FA',
            restart_warning: '重启面板将断开所有连接,确定继续吗?',
            reset_2fa_success: '2FA已重置',
            reset_2fa_instruction: '请使用认证器扫描新的二维码',
            restarting: '面板正在重启...',
            validation_error: '验证失败',
            reinstall: '重新安装 MC 服务器',
            reinstall_confirm: '这样做会删除当前 MC 服务器的所有文件（包括地图和配置），确定要这么做吗？',
            reinstall_success: '重置成功，即将进入引导界面',
            ai_settings: 'AI 设置',
            ai_endpoint: 'API 域名',
            ai_key: 'API Key',
            ai_model: '模型名称',
            ai_endpoint_desc: 'OpenAI 兼容接口地址, 如 https://api.openai.com/v1',
            ai_key_desc: '如果接口不需要 Key 可留空',
            ai_model_placeholder: '例如 gpt-3.5-turbo',
            ai_test: '测试连接',
            ai_test_success: '连接成功',
            ai_test_fail: '连接失败',
            webhook_settings: '事件订阅 (Webhook)',
            webhook_urls: 'Webhook 接收端点 (每行一个)',
            webhook_desc: '当服务器启动、停止、崩溃或有玩家进出游戏时，面板会向这些 URL 发送 POST 事件通知。',
            console_info_position: '控制台信息展示位置',
            pos_top: '顶部显示',
            pos_sidebar: '侧边栏显示',
            pos_hide: '不显示',
            github_proxy: 'GitHub 代理加速',
            github_proxy_desc: '国内用户可选择代理以加速系统更新与 Java 下载',
            backup_maintenance: '备份与维护',
            create_global_backup: '创建全局备份',
            backup_include: '包含内容',
            backup_configs: '面板配置',
            backup_java: 'Java 环境',
            backup_instances: '游戏实例',
            restore_global_confirm: '确定要还原备份 {name} 吗？这会覆盖当前所有配置和实例，完成后面板将重启。',
            restoring: '正在应用备份，面板即将重启...',
            import_backup: '导入备份',
            no_instances: '没有发现实例',
            no_java: '没有从面板下载的 Java',
            appearance: '外观自定义',
            custom_logo: '自定义 Logo',
            logo_desc: '建议使用正方形图片，将替换登录页和实例详情页的默认图标',
            background_image: '背景图片',
            background_desc: '设置面板的背景图片，将覆盖默认背景色',
            sidebar_opacity: '侧边栏透明度',
            content_opacity: '内容区域透明度',
            card_opacity: '卡片透明度',
            login_opacity: '登录界面透明度',
            instance_opacity: '实例列表透明度',
            appearance_preview: '效果预览',
            tab_general: '通用',
            tab_login: '登录界面',
            tab_instance_list: '实例列表',
            tab_instance_detail: '实例详情',
            tab_other: '其他界面',
            other_interfaces_desc: 'Java管理界面、插件加载界面、FRP管理器等扩展界面将继承通用外观设置。如需单独调整，请在此处修改。',
            upload: '上传',
            upload_success: '上传成功',
            account_mgmt: '账号管理',
            admin_user: '管理员用户名',
            current_pass: '当前密码 (必填)',
            new_pass: '新密码 (留空则不修改)',
            confirm_new_pass: '确认新密码',
            disable_2fa: '禁用 2FA 双重认证',
            enable_2fa: '启用 2FA 双重认证',
            disable_2fa_confirm: '确定要禁用 2FA 双重认证吗？这会降低账号的安全性。',
            account_update_success: '管理员账号信息更新成功',
            account_update_fail: '账号信息更新失败'
        },

        panel_backup: {
            title: '面板备份',
            description: '管理整个面板的数据备份、系统配置、Java环境及实例归档',
            create_btn: '创建全局备份',
            import_btn: '导入备份',
            restore_btn: '回档此备份',
            restore_title: '系统回档与恢复',
            backup_content: '包含内容',
            opt_config: '系统配置',
            prompt_note: '备份备注（可选）',
            backup_include: '选择备份内容',
            backup_configs: '系统与面板核心配置文件 (data/*.json)',
            backup_java: '已安装的 Java 环境',
            backup_instances: 'Minecraft 实例数据',
            no_instances: '暂无已创建的实例',
            no_java: '暂无通过面板安装的 Java 环境',
            restore_confirm: '确定要将系统回档至 {name} 吗？',
            ask_restore_now: '备份文件 {name} 导入成功！是否立即回档至此备份？',
            restoring: '正在安全回档中，请稍候...',
            confirm_delete_msg: '确定要永久删除备份归档 {name} 吗？此操作不可逆。',
            uploading: '正在上传备份文件...',
            import_success: '备份导入成功！'
        },

        plugins: {
            title: '插件管理',
            description: '管理和配置面板插件，扩展面板功能',
            official: '官方插件',
            official_badge: '官方',
            third_party: '第三方插件',
            running: '运行中',
            stopped: '已停止',
            error: '加载错误',
            install: '安装插件',
            upload_desc: '请上传 .zip 格式的插件包。插件将被上传到服务器临时目录并自动解压安装。',
            update: '更新插件',
            update_title: '更新插件: {name}',
            install_confirm_title: '确认安装插件',
            update_badge: '更新',
            new_badge: '新插件',
            update_comparison: '版本对比',
            current_version: '当前版本',
            new_version: '准备安装',
            downgrade_warning: '注意：准备安装的版本低于当前已安装版本。',
            install_confirm_hint: '请确认以上插件信息，点击下方确认按钮开始安装。',
            confirm_update: '确认更新',
            confirm_install: '确认安装',
            upload_analyze: '上传并解析',
            no_plugins: '暂无插件',
            no_plugins_hint: '安装插件以扩展面板功能',
            enabled_success: '{name} 已启用',
            disabled_success: '{name} 已禁用',
            uninstall_title: '卸载插件',
            uninstall_confirm: '确定要卸载插件 {name} 吗？',
            uninstall_warning_hint: '卸载插件将永久删除其所有文件和数据，此操作不可恢复。',
            type_to_confirm_hint: '请输入插件名称 {name} 以确认删除：',
            type_to_confirm: '输入插件名称以确认',
            confirmation_mismatch: '输入的名称不匹配',
            uninstalled_success: '{name} 已卸载',
            err_invalid_zip: '插件文件不是有效的 ZIP 压缩包',
            err_manifest_not_found: '找不到插件清单文件 (plugin.json)',
            err_missing_id: '插件清单缺少 "id" 字段',
            err_install_info: '缺少安装信息',
            install_title: '安装插件',
            install_warning_title: '安全警告',
            install_warning_text: '第三方插件可能包含恶意代码，请在信任来源的情况下安装。插件将获得执行系统命令、访问文件系统等高权限。',
            install_path_label: '插件路径',
            install_path_placeholder: '输入插件目录或 .zip 文件的绝对路径',
            install_path_hint: '插件目录需包含 plugin.json 清单文件',
            install_file_label: '上传插件包',
            install_file_hint: '支持 .zip 格式的插件包，需包含 plugin.json',
            install_button: '安装',
            no_install_source: '请提供插件路径或上传插件包',
            install_success: '插件安装成功',
            update_success: '插件更新成功',
            update_button: '更新',
            guide_button: '开发指南',
            market_button: '插件商店',
            guide_title: '插件开发指南',
            guide_desc: '学习如何为云语面板开发自定义插件',
            guide_step1_title: '快速创建',
            guide_step1_desc: '创建一个包含 plugin.json 的文件夹即可开始。',
            guide_step2_title: '编写代码',
            guide_step2_desc: '使用 JavaScript 编写后端逻辑与前端界面。',
            guide_step3_title: '打包发布',
            guide_step3_desc: '将文件夹压缩为 .zip 包即可在面板中安装。',
            guide_structure_title: '标准目录结构',
            guide_structure_desc: '一个标准的插件包应包含以下文件：',
            guide_file_json: '清单文件 (必须)',
            guide_file_main: '后端入口',
            guide_file_public: '静态资源目录',
            guide_file_frontend: '前端入口',
            guide_toc: '文档目录',
            guide_export: '导出为 MD',
            export_md: '导出为 MD 文档',
            intro: '快速开始',
            manifest: 'plugin.json 参考',
            dependencies: 'NPM 依赖管理',
            backend: '后端开发与 API',
            frontend: '前端开发',
            'vue-options': 'Vue 组件选项与内置组件',
            i18n: '国际化 (i18n)',
            socket: 'Socket.IO 通信',
            lifecycle: '插件生命周期',
            'panel-api': '面板内置 API 端点',
            css: 'CSS 样式类参考',
            tips: '开发技巧',
            tasks: '常驻任务与定时任务',
            permissions: '权限系统',
            'plugin-settings': '插件设置系统',
            storage: 'KV 存储',
            events: '插件间通信',
            status: '状态监控',
            'plugin-deps': '插件依赖声明',
            'api-version': 'API 版本化',
            disclaimer_title: '插件安全免责声明',
            disclaimer_line1: '插件系统具备较高权限，包括但不限于执行系统命令、查看系统状态、运行外部程序及操作文件系统等能力。',
            disclaimer_line2: '安装第三方插件前，请务必确认插件来源可信。因使用第三方插件导致的任何数据丢失、系统损坏或安全问题，本面板不承担任何责任。',
            disclaimer_line3: '使用本插件系统即表示您已了解并同意上述风险，所有因插件使用产生的后果由用户自行承担。',
            disclaimer_confirm: '我已阅读并理解上述风险提示与免责声明',
            disclaimer_agree: '确认并继续',
            settings: '设置',
            settings_saved: '设置已保存',
            no_settings: '该插件未注册设置项',
            permissions_title: '权限声明',
            plugin_dependencies: '插件依赖',
        },

        setup: {
            welcome: '欢迎使用 MC Panel',
            desc: '检测到您的服务器目录为空。请按照指引安装一个新的 Minecraft 服务器。',
            select_loader_type: '选择加载器类型',
            select_mc: '选择游戏版本',
            select_loader: '选择加载器版本',
            choose: '请选择...',
            install: '一键安装',
            installing: '正在安装...',
            install_start_toast: '开始安装，请稍候...',
            install_success_toast: '安装成功！即将刷新...',
            install_fail_toast: '安装失败',
            install_timeout_toast: '安装超时，请检查日志',
            fetch_fail_mc: '获取游戏版本失败',
            fetch_fail_loader: '获取加载器版本失败',
            loading_versions: '正在获取版本列表...',
            restore_from_backup: '从备份恢复',
            restoring_uploading: '正在上传备份...',
            restoring_applying: '正在应用备份，面板即将重启...'
        },
        java: {
            title: 'Java 管理',
            installed: '已安装',
            online_install: '在线安装',
            detect_system: '检测系统 Java',
            add_local: '添加本地 Java',
            local_path_placeholder: '输入 Java 可执行文件路径，如 /usr/bin/java',
            local_path_desc: '指定本地已安装的 Java 可执行文件路径',
            no_installed: '暂无已安装的 Java 版本',
            no_available: '无法获取可用版本，请检查网络连接',
            fetching: '正在获取可用版本...',
            fetch_fail: '获取可用版本失败',
            install: '安装',
            already_installed: '已安装',
            confirm_remove: '确定要删除 {name} 吗？此操作不可撤销。',
            system_not_found: '未检测到系统 Java',
            system_found: '检测到系统 Java',
            add_to_list: '是否添加到管理列表？',
            task_running: '已有正在进行的安装任务'
        },
        instance_manager: {
            title: '实例管理',
            create_btn: '创建新实例',
            manage_java: '管理 Java',
            search_placeholder: '搜索实例...',
            state_starting: '启动中',
            state_running: '运行中',
            state_stopping: '停止中',
            state_stopped: '已停止',
            online_players: '人在线',
            select_btn: '进入面板',
            settings_btn: '实例设置',
            delete_btn: '删除实例',
            create_modal_title: '创建新实例',
            edit_modal_title: '修改实例设置',
            name_label: '实例名称',
            name_placeholder: '例如：红石生存',
            jar_placeholder: 'server.jar (可选)',
            java_path_label: 'Java 路径',
            java_args_label: 'JVM 参数',
            java_args_placeholder: '-Xms1G -Xmx4G',
            java_args_recommend_tip: '系统总内存 {total}G，建议最大内存配置为 -Xmx{recommend}G (如 -Xmx{recommend}G)',
            forge_jar_tip: 'Forge/NeoForge 的启动 jar 通常由安装器自动生成，如不确定请勿随意修改',
            run_sh_mode: 'run.sh 模式',
            jar_mode: 'jar 模式',
            user_jvm_args_tip: 'JVM 参数将在启动时自动写入 user_jvm_args.txt 文件',
            delete_confirm: '确定要删除实例 "{name}" 吗？此操作将永久删除该实例的所有数据且不可撤销！',
            create_success: '实例创建成功',
            update_success: '设置已更新',
            delete_success: '实例已删除',
            back_to_list: '返回实例列表',
            no_instances: '暂无实例，点击右上角创建',
            action_sent: '实例 {name}: {action} 指令已发送',
            select_fail: '选择实例失败',
            instance_count: '{count} 个实例'
        },

        frp: {
            title: 'FRP 内网穿透',
            install_manage: '安装管理',
            not_installed: 'FRP 客户端未安装',
            install: '安装',
            install_first: '请先安装 FRP 客户端',
            installing: '正在安装...',
            installed: '已安装',
            version: '版本',
            uninstall: '卸载',
            uninstall_confirm: '确定要卸载 FRP 客户端吗？这将删除 frpc 二进制文件（配置文件会保留）。',
            config: '连接配置',
            config_list: '配置列表',
            new_config: '新建配置',
            edit_config: '编辑配置',
            config_name: '配置名称',
            config_name_placeholder: '例如：我的服务器',
            config_name_required: '请填写配置名称',
            no_configs: '暂无配置',
            no_configs_hint: '创建一个 FRP 配置来开始内网穿透',
            delete_config_confirm: '确定要删除配置 "{name}" 吗？如果正在运行将会被停止。',
            gui_mode: '图形模式',
            text_mode: '文本模式',
            server_addr: '服务端地址',
            server_port: '服务端端口',
            auth_token: '认证密钥',
            tunnels: '隧道列表',
            tunnel_name: '隧道名称',
            tunnel_type: '协议类型',
            local_ip: '本地 IP',
            local_port: '本地端口',
            remote_port: '远程端口',
            add_tunnel: '添加隧道',
            no_tunnels: '暂无隧道，点击上方按钮添加',
            start: '启动',
            stop: '停止',
            restart: '重启',
            running: '运行中',
            stopped: '已停止',
            logs: '运行日志',
            save_config: '保存配置',
            config_saved: '配置已保存',
            version_manager: '版本管理',
            switch_version: '安装/更换',
            no_releases: '无法获取版本信息，请检查网络',
            fetch_releases: '获取版本列表...',
            latest: '最新版',
            server_addr_desc: '填写 FRP 服务端 (frps) 的公网 IP 或域名',
            server_port_desc: '对应 frps.toml 中的 bindPort',
            auth_token_desc: '需与服务端配置的 token 一致',
            started_success: 'FRP 已启动',
            stopped_success: 'FRP 已停止',
            start_fail: '启动失败',
            use_github_proxy: '使用 GitHub 代理加速',
            proxy_url: '代理加速地址',
            proxy_url_placeholder: '例如：https://ghfast.top/ 或 https://mirror.ghproxy.com/',
            proxy_tips: '国内网络下载慢或超时可开启代理加速',
            proxy_presets: '常用预设',
            import_config: '导入配置',
            config_format: '配置格式',
            format_toml: 'TOML 格式 (.toml)',
            format_ini: 'INI 格式 (.ini)',
            import_file: '导入文件',
            import_file_tip: '支持导入 .ini、.toml、.conf、.txt 等内网穿透配置文件',
            import_clipboard: '剪贴板导入',
            import_clipboard_tip: '一键粘贴来自内网穿透平台的配置内容',
            clipboard_modal_title: '从剪贴板粘贴配置',
            clipboard_modal_desc: '请将内网穿透平台（如 SakuraFrp、ME Frp、OpenFrp 等）提供的配置内容粘贴到下方：',
            clipboard_modal_confirm: '确认导入',
            import_success: '导入成功 (已识别为 {format} 格式)',
            import_fail: '导入失败：未能解析有效配置',
            clipboard_empty: '剪贴板内容为空',
            clipboard_denied: '浏览器未授予剪贴板读取权限，已打开手动粘贴输入框',
            format_switch_tip: '切换格式时将自动转换当前配置内容',
            user: '用户标识 (User)',
            user_desc: '部分穿透平台需要的用户标识 (如 SakuraFrp 的 user 字段)',
            clear_logs: '清空日志',
            logs_cleared: '日志已清空',
            copy_logs: '复制日志',
            logs_copied: '日志已复制到剪贴板',
            auto_scroll: '自动滚动',
            back_to_config: '返回配置'
        }
    },
    en: {
        common: {
            cancel: 'Cancel',
            confirm: 'Confirm',
            execute: 'Execute',
            save: 'Save',
            danger_zone: 'Danger Zone',
            delete: 'Delete',
            edit: 'Edit',
            rename: 'Rename',
            search: 'Search',
            filter: 'Filter',
            back: 'Back',
            loading: 'Loading...',
            load_more: 'Load More',
            success: 'Success',
            error: 'Failed',
            delete_confirm: 'Delete {count} items?',
            yes: 'Yes',
            no: 'No',
            enabled: 'Enabled',
            disabled: 'Disabled',
            unknown: 'Unknown',
            actions: 'Actions',
            name: 'Name',
            size: 'Size',
            time: 'Time',
            status: 'Status',
            switchTheme: 'Switch Theme',
            switchLang: '切换语言',
            logout: 'Logout',
            upload: 'Upload',
            download: 'Download',
            refresh: 'Refresh',
            close: 'Close',
            add: 'Add',
            remove: 'Remove',
            mode_gui: 'GUI Mode',
            mode_text: 'Text Mode',
            gui_mode: 'GUI Mode',
            text_mode: 'Text Mode',
            player: 'Player',
            player_name: 'Player Name',
            action_sent: 'Action Sent',
            select_fail: 'Selection Failed',
            save_fail: 'Save Failed',
            server_offline: 'Server is offline',
            note: 'Note',
            lock: 'Lock',
            unlock: 'Unlock',
            locked: 'Locked',
            edit_note: 'Edit Note',
            processing: 'Processing...',
            no_data: 'No data',
            reset: 'Reset',
            install: 'Install',
            installing: 'Installing...',
            dont_show_again: "Don't show again",
            unsaved_changes: 'You have unsaved changes. Are you sure you want to leave?',
            export: 'Export'
        },
        cancel: 'Cancel',
        close: 'Close',
        login: {
            title: 'MC Panel',
            prompt_scan: 'Use Google Authenticator to scan this QR code.',
            placeholder_code: 'Enter 2FA Code',
            btn_verify: 'Verify Identity',
            toast_fail: 'Invalid Code',
            toast_error: 'Request Failed',
            placeholder_user: 'Enter Username',
            placeholder_pass: 'Enter Password',
            placeholder_captcha: 'Captcha',
            slide_to_verify: 'Security Verification',
            click_to_verify: 'Click to Verify',
            verified_success: 'Verification Passed',
            captcha_modal_title: 'Security Verification',
            drag_tip: 'Drag slider to complete the puzzle',
            refresh_captcha: 'Refresh',
            verify_failed: 'Puzzle not aligned, please try again',
            loading_captcha: 'Loading verification...',
            placeholder_confirm_pass: 'Confirm New Password',
            btn_login: 'Login',
            btn_next: 'Next',
            btn_prev: 'Back',
            enable_2fa: 'Enable 2FA (Recommended)',
            init_title: 'Initialize Admin Account',
            btn_init: 'Initialize',
            toast_init_success: 'Admin credentials initialized successfully',
            toast_init_fail: 'Initialization failed',
            toast_login_fail: 'Incorrect username or password',
            toast_captcha_fail: 'Invalid captcha',
            locked_prefix: 'Too many failed attempts. Please try again after ',
            locked_suffix: ' seconds',
            mismatch_pass: 'Passwords do not match',
            switch_to_2fa: 'Login with 2FA Token',
            switch_to_password: 'Login with Username/Password',
            disaster_recovery: 'Disaster Recovery / Restore Backup',
            recovery_title: 'Disaster Recovery',
            recovery_desc: 'If the panel is corrupted or inaccessible, you can restore directly from a global backup zip by providing the admin password.',
            recovery_pass_label: 'Admin Password',
            recovery_pass_placeholder: 'Enter admin password to authorize restore',
            recovery_file_label: 'Backup Archive (.zip)',
            recovery_select_file: 'Select Backup Archive (.zip)',
            recovery_btn_start: 'Verify & Start Restore',
            recovery_btn_cancel: 'Back to Login',
            recovery_no_pass: 'Please enter the admin password',
            recovery_no_file: 'Please select a backup archive to restore'
        },
        sidebar: {
            title: 'MC Panel',
            dashboard: 'Dashboard',
            settings: 'Settings',
            mods: 'Mods',
            files: 'Files',
            backups: 'Backups',
            auth: 'Auth',
            voicechat: 'Voice Chat',
            players: 'Players',
            modrinth: 'Mod Download',
            java: 'Java',
            about: 'About',
            plugins: 'Plugins',
            frp_manager: 'FRP Tunnel',
            scrolls: 'Scroll Workshop',
            customize: 'Customize Sidebar',
            customize_desc: 'Adjust visibility and order of sidebar items. Hidden items can still be accessed through other means.',
            reset: 'Reset to Default'
        },
        scrolls: {
            title: 'Scrolls',
            description: 'Manage and configure console event-driven automation scrolls for your server',
            guide_button: 'Dev Guide',
            market_button: 'Official Store',
            install: 'Install Scroll',
            market_link: 'Visit Scroll Market',
            market_hint: 'Looking for more crash-proof automation? Visit the official Scroll Market.',
            market_title: 'Discover More Official & Community Scrolls?',
            market_desc: 'Browse and download auto-cleaner, anti-addiction, fast-sleep and more from GitHub.',
            official_market: 'Official Scrolls Market',
            disclaimer_title: 'Scroll Security Disclaimer',
            disclaimer_line1: 'The scroll system executes via Minecraft server console commands and events, with permissions to dispatch console commands, broadcast messages, and manage instance data.',
            disclaimer_line2: 'Before installing third-party scrolls, please ensure the source is trustworthy. The panel is not responsible for any server crashes, world corruption, or data loss caused by third-party scripts.',
            disclaimer_line3: 'By using this scroll system, you acknowledge and accept the risks. All consequences arising from running scrolls are borne by the user.',
            disclaimer_confirm: 'I have read and understand the risk warnings and disclaimer',
            disclaimer_agree: 'Confirm and Continue',
            upload_desc: 'Please upload a .zip scroll package. The panel will parse the package structure and manifest.',
            install_file_label: 'Choose Scroll ZIP File',
            install_confirm_title: 'Confirm Scroll Installation',
            update_title: 'Update Scroll: {name}',
            new_badge: 'New Scroll',
            update_badge: 'Update',
            update_comparison: 'Version Comparison',
            current_version: 'Current',
            new_version: 'Target',
            downgrade_warning: 'Warning: The target version is lower than the current installed version.',
            install_confirm_hint: 'Please review the scroll information above and click confirm to proceed with installation.',
            confirm_update: 'Confirm Update',
            confirm_install: 'Confirm Installation',
            upload_analyze: 'Upload & Analyze',
            analyzing: 'Analyzing...',
            installed_success_toast: 'Scroll {name} installed successfully!',
            config_schema_title: 'Configurable Options',
            running: 'Running',
            stopped: 'Stopped',
            error: 'Error',
            no_scrolls: 'No Scrolls Installed',
            no_scrolls_hint: 'Download scroll ZIPs from the Market or create your own to upload',
            settings: 'Settings',
            save_config: 'Save Config',
            config_saved: 'Configuration saved and hot-reloaded!',
            export: 'Export',
            delete: 'Delete',
            delete_title: 'Uninstall Scroll',
            delete_confirm: 'Are you sure you want to delete scroll {name}?',
            delete_warning_hint: 'Deleting this scroll will permanently remove all its files and persistent data.',
            deleted_success: '{name} uninstalled successfully'
        },
        about: {
            title: 'About MWP',
            description: 'A lightweight, modern Fabric server management panel.',
            version: 'Version',
            author: 'Author',
            version: 'Version',
            author: 'Author',
            source: 'Source Code',
            website: 'Website',
            check_update: 'Check Update',
            checking: 'Checking...',
            update_available: 'Update Available!',
            latest_version: 'Up to date',
            new_version: 'New Version',
            download: 'Download',
            update_now: 'Update Now',
            updating: 'Updating...',
            current_is_latest: 'You are using the latest version.',
            released_at: 'Released',
            show_changelog: 'View Changelog',
            hide_changelog: 'Hide Changelog'
        },
        dashboard: {
            console_title: 'Console',
            start: 'Start Server',
            stop: 'Stop Server',
            force_stop: 'Force Kill',
            force_stop_confirm_title: 'Force Kill Server',
            force_stop_confirm_msg: 'Are you sure you want to force kill the server?<br><strong class="text-danger">This will immediately terminate the server process, which may cause data loss!</strong><br>Only use this when the server cannot be stopped normally.',
            force_stop_sent: 'Force kill command sent',
            state_starting: 'Starting',
            state_running: 'Running',
            state_stopping: 'Stopping',
            state_stopped: 'Stopped',
            server_info: 'Minecraft Server',
            online_players: 'Online Players',
            system_resource: 'System Load',
            cpu_usage: 'CPU Usage',
            ram_usage: 'RAM Usage',
            mem_usage: 'Memory',
            swap_usage: 'Swap',
            disk_usage: 'Disk',
            swap_unconfigured: 'Not Configured',
            status: 'Status',
            status_normal: 'Normal',
            status_high: 'High Load',
            send_cmd_placeholder: 'Send command...',
            send: 'Send',
            toast_sent: 'Command Sent',
            system: 'System',
            no_players: 'No Players',
            target: 'Game Version',
            loader: 'Server Core',
            java_version: 'Java Runtime',
            port: 'Server Port',
            collapse_info: 'Collapse',
            expand_info: 'Expand',
            cmd_helper: 'Command Helper',
            cmd_all: 'All',
            cmd_cat_player: 'Player',
            cmd_cat_server: 'Server',
            cmd_cat_world: 'World',
            cmd_cat_admin: 'Admin',
            cmd_cat_advanced: 'Advanced',
            cmd_search: 'Search commands...',
            cmd_quick: 'Quick Command',
            cmd_quick_title: 'Quick Send',
            cmd_no_result: 'No matching commands found',
            cmd_advancement_desc: 'Grant or revoke advancements',
            cmd_attribute_desc: 'Query or modify entity attributes',
            cmd_clear_desc: 'Clear player inventory',
            cmd_clone_desc: 'Clone blocks in a region',
            cmd_damage_desc: 'Deal damage to an entity',
            cmd_data_desc: 'Get/modify block or entity NBT data',
            cmd_datapack_desc: 'Manage datapacks',
            cmd_debug_desc: 'Debug performance profiling',
            cmd_defaultgamemode_desc: 'Set default game mode',
            cmd_difficulty_desc: 'Set game difficulty',
            cmd_effect_desc: 'Give or clear potion effects',
            cmd_enchant_desc: 'Enchant held item',
            cmd_experience_desc: 'Add/set/query player experience',
            cmd_fill_desc: 'Fill a region with blocks',
            cmd_fillbiome_desc: 'Fill a region with a biome',
            cmd_forceload_desc: 'Force load/unload chunks',
            cmd_function_desc: 'Run a function',
            cmd_gamemode_desc: 'Set player game mode',
            cmd_gamerule_desc: 'Query or modify game rules',
            cmd_give_desc: 'Give items to player',
            cmd_help_desc: 'Show help info',
            cmd_item_desc: 'Modify/replace inventory items',
            cmd_kick_desc: 'Kick player from server',
            cmd_kill_desc: 'Kill entities',
            cmd_list_desc: 'List online players',
            cmd_locate_desc: 'Locate nearest structure/biome',
            cmd_loot_desc: 'Give loot table items',
            cmd_msg_desc: 'Send private message to player',
            cmd_particle_desc: 'Create particle effects',
            cmd_place_desc: 'Place structures/features',
            cmd_playsound_desc: 'Play a sound',
            cmd_random_desc: 'Random number generation',
            cmd_reload_desc: 'Reload loot tables/functions etc.',
            cmd_recipe_desc: 'Give/remove player recipes',
            cmd_ride_desc: 'Mount/dismount entities',
            cmd_rotate_desc: 'Rotate entity facing',
            cmd_say_desc: 'Broadcast message to all players',
            cmd_schedule_desc: 'Schedule function execution',
            cmd_scoreboard_desc: 'Manage scoreboards',
            cmd_seed_desc: 'Display world seed',
            cmd_setblock_desc: 'Place block at position',
            cmd_setworldspawn_desc: 'Set world spawn point',
            cmd_spawnpoint_desc: 'Set player spawn point',
            cmd_spectate_desc: 'Spectate a player',
            cmd_spreadplayers_desc: 'Spread players across area',
            cmd_stopsound_desc: 'Stop playing sounds',
            cmd_summon_desc: 'Summon an entity',
            cmd_tag_desc: 'Manage entity tags',
            cmd_team_desc: 'Manage teams',
            cmd_teammsg_desc: 'Send message to team',
            cmd_teleport_desc: 'Teleport entities',
            cmd_tellraw_desc: 'Send JSON message to player',
            cmd_time_desc: 'Query/set game time',
            cmd_title_desc: 'Display title to player',
            cmd_trigger_desc: 'Modify trigger scoreboard',
            cmd_weather_desc: 'Set weather',
            cmd_worldborder_desc: 'Manage world border',
            cmd_ban_desc: 'Ban a player',
            cmd_ban_ip_desc: 'Ban an IP address',
            cmd_banlist_desc: 'View ban list',
            cmd_deop_desc: 'Revoke operator status',
            cmd_op_desc: 'Grant operator status',
            cmd_pardon_desc: 'Unban a player',
            cmd_pardon_ip_desc: 'Unban an IP address',
            cmd_save_all_desc: 'Save world to disk',
            cmd_save_off_desc: 'Disable auto-saving',
            cmd_save_on_desc: 'Enable auto-saving',
            cmd_setidletimeout_desc: 'Set idle kick timeout',
            cmd_stop_desc: 'Stop the server',
            cmd_whitelist_desc: 'Manage whitelist',
            cmd_execute_desc: 'Execute command conditionally',
            cmd_bossbar_desc: 'Manage boss bars',
            cmd_tick_desc: 'Query/modify game tick rate',
            cmd_tp_desc: 'Teleport entities to destination or coordinates',
            cmd_tell_desc: 'Send private message to player',
            cmd_w_desc: 'Send whisper message to player',
            cmd_xp_desc: 'Add, set, or query player experience',
            cmd_me_desc: 'Send action message to chat',
            cmd_perf_desc: 'Server performance profiling',
            cmd_jfr_desc: 'Java Flight Recorder profiling',
            cmd_publish_desc: 'Publish world to LAN',
            cmd_return_desc: 'Return value from function'
        },
        files: {
            path: 'Path',
            operate: 'Batch Operation',
            new_folder: 'New Folder',
            new_file: 'New File',
            go_up: 'Go Up',
            upload_file: 'Upload File',
            upload_folder: 'Upload Folder',
            upload_tips: 'Drag & Drop files to upload',
            total: '{count} items',
            move: 'Move',
            copy: 'Copy',
            compress: 'Compress',
            extract: 'Extract',
            drop_to_upload: 'Drop files here to upload',
            disable: 'Disable',
            enable: 'Enable',
            modal_move_title: 'Move/Copy Files',
            modal_move_dest: 'Destination (relative to root)',
            modal_compress_title: 'Compress Files',
            modal_compress_name: 'Archive Name (.zip)',
            modal_new_folder: 'Folder Name',
            modal_new_file: 'File Name',
            modal_rename: 'New Name',
            modal_edit_warning_title: 'Warning',
            modal_edit_warning_msg: 'File {name} may not be text. Edit anyway?',
            file_not_found: 'File {name} not found, please check if it exists',
            folder_not_found: 'Folder {name} not found, please check if it exists',
            clipboard_msg: '{action} {count} file(s)',
            extract_to: 'Extract to',
            extract_here: 'Extract here',
            extract_to_folder: 'Extract to folder',
            modal_extract_title: 'Extract File',
            modal_extract_dest: 'Destination (leave empty to extract to same-name folder)',
            preview_image: 'Preview Image',
            preview_archive: 'Preview Archive',
            compressing: 'Compressing...',
            extracting: 'Extracting...',
            upload_confirm_title: 'Confirm Uploading Files',
            upload_confirm_desc: 'Please confirm and select the files to upload (all items are checked by default):',
            upload_list_title: 'Pending Files List',
            error_read: 'Failed to read file',
            editor: {
                unsaved: 'Unsaved',
                saved: 'Saved',
                search: 'Find',
                search_tip: 'Find & Replace (Ctrl+F / Ctrl+H)',
                search_placeholder: 'Find...',
                replace: 'Replace',
                replace_all: 'Replace All',
                replace_placeholder: 'Replace with...',
                find_prev: 'Previous',
                find_next: 'Next',
                no_match: 'No matches',
                replace_count: 'Successfully replaced {count} occurrence(s)',
                format: 'Format',
                format_tip: 'Format / Beautify (JSON/Code)',
                format_success: 'Code formatted successfully',
                already_formatted: 'Content is already well formatted',
                format_json_fail: 'JSON syntax error, cannot format',
                goto_line: 'Go to',
                goto_tip: 'Go to line',
                goto_prompt: 'Enter line number to go to (1 - {max}):',
                wrap_on: 'Wrap',
                wrap_off: 'No Wrap',
                wrap_tip: 'Toggle word wrap',
                font_size_tip: 'Toggle font size',
                fullscreen: 'Fullscreen',
                exit_fullscreen: 'Exit Fullscreen',
                lines: 'Lines',
                length: 'Characters',
                modified: 'Modified'
            }
        },
        mods: {
            title: 'Mods List',
            source_local: 'Local Parsed',
            file_info: 'Mod & File Details',
            filename: 'File Name',
            filesize: 'File Size',
            no_desc: 'No description provided for this mod.',
            upload_mod: 'Upload Mod',
            upload_placeholder: 'Click or Drag .jar files here',
            search_placeholder: 'Search mods...',
            version: 'Version',
            empty: 'No mods found',
            browse_modrinth: 'Browse Modrinth',
            search_modrinth: 'Search Modrinth...',
            install: 'Install',
            installing: 'Installing...',
            download_success: 'Mod {name} downloaded successfully',
            download_fail: 'Download failed: {error}',
            select_version: 'Select Version',
            no_versions: 'No compatible versions found',
            project_info: 'Info',
            loader: 'Loader',
            game_version: 'Game Version',
            description: 'Description',
            categories_title: 'Categories',
            categories: {
                adventure: 'Adventure',
                cursed: 'Cursed',
                decoration: 'Decoration',
                economy: 'Economy',
                equipment: 'Equipment',
                food: 'Food',
                library: 'Library',
                magic: 'Magic',
                optimization: 'Optimization',
                storage: 'Storage',
                utility: 'Utility',
                worldgen: 'World Generation',
                technology: 'Technology',
                fabric: 'Fabric',
                forge: 'Forge',
                quilt: 'Quilt',
                neoforge: 'NeoForge',
                canvas: 'Canvas',
                social: 'Social',
                transportation: 'Transportation',
                management: 'Management',
                'game-mechanics': 'Game Mechanics',
                minigame: 'Minigame',
                mobs: 'Mobs'
            },
            modrinth: {
                project_info: 'Basic Info',
                introduction: 'Mod Introduction',
                found_versions: 'Found {count} versions',
                project_type: 'Project Type',
                environment: 'Environment',
                env_server: 'Server',
                env_client: 'Client',
                sort: 'Sort By',
                types: {
                    mod: 'Mod',
                    plugin: 'Plugin',
                    datapack: 'Datapack',
                    resourcepack: 'Resource Pack'
                },
                env: {
                    server: 'Server Support',
                    client: 'Client Required'
                },
                sorting: {
                    relevance: 'Relevance',
                    downloads: 'Downloads',
                    follows: 'Follows',
                    newest: 'Newest',
                    updated: 'Recently Updated'
                },
                last_updated: 'Updated',
                env_required: 'Required',
                env_optional: 'Optional',
                env_unsupported: 'Unsupported',
                searching: 'Searching Modrinth...',
                no_results: 'No results found',
                reset_filters: 'Reset All Filters',
                try_different: 'Try different filters or a broader search term'
            },
            translate: 'AI Translate',
            translating: 'Translating...',
            translate_fail: 'AI translation failed',
            translate_fail_config: 'AI config invalid, check Panel Settings',
            upload_file: 'Upload File',
            upload_folder: 'Upload Folder',
            upload_confirm_title: 'Confirm Uploading Mod Files',
            upload_confirm_desc: 'Detected that your uploaded files contain folders or non-.jar files, please confirm the files to upload below:',
            compliant_files: 'Compliant Files (.jar)',
            no_compliant: 'No compliant files',
            non_compliant_files: 'Non-compliant Files',
            no_non_compliant: 'No non-compliant files',
            is_folder: 'Folder',
            is_non_jar: 'Non-JAR',
            conflict_btn: 'Conflict Mods',
            conflict_title: 'Resolve Mod Version Conflicts',
            conflict_list: 'Conflict Mods List (Total {count})',
            versions_count: '{count} versions',
            earliest_time: 'Earliest: {time}',
            conflict_desc: 'Please select a version you wish to keep. Other versions will be automatically deleted to restore normal loading.',
            file_size: 'File Size: {size}',
            mtime: 'Modified Time: {time}',
            metadata_version: 'Mod Version: {version}',
            keep_this_version: 'Keep This Version',
            no_conflicts: 'No conflicting mod versions detected at the moment.',
            skip_confirm: 'Do not show confirmation dialog after selecting a version',
            confirm_keep_title: 'Confirm Keeping Version',
            confirm_keep_msg: 'Are you sure you want to keep mod version [{name}] and delete the other {count} old/conflicting versions?',
            no_info: 'No additional information available for this mod.'
        },

        players: {
            title: 'Player Manager',
            online_players: 'Online Players',
            no_online: 'No players online',
            empty: 'No players found',
            search_player: 'Search player...',
            player_name: 'Player Name',
            whitelist: 'Whitelist',
            ops: 'Operators (OP)',
            bans: 'Banned Players',
            kick: 'Kick',
            ban: 'Ban',
            teleport: 'Teleport',
            gamemode: 'Gamemode',
            clear_inv: 'Clear Inv',
            kill: 'Kill',
            reason: 'Reason',
            expires: 'Expires',
            permanent: 'Permanent',
            more_actions: 'More Actions',
            give_item: 'Give Item',
            give_item_prompt: 'Item ID Count (e.g. minecraft:diamond 1)',
            search_item_placeholder: 'Search item name or ID...',
            selected_item: 'Selected Item',
            no_selection: 'Select an item',
            custom_item_id_label: 'Manually enter custom Item ID',
            quantity_label: 'Quantity',
            qty_unit: 'pcs',
            tab_building_blocks: 'Building',
            tab_decorations: 'Decoration',
            tab_redstone: 'Redstone',
            tab_transportation: 'Transportation',
            tab_tools: 'Tools',
            tab_combat: 'Combat',
            tab_food: 'Food',
            tab_materials: 'Materials',
            tab_brewing: 'Brewing',
            tab_misc: 'Misc',
            effect: 'Give Effect',
            effect_prompt: 'Effect ID Duration(s) Level (e.g. minecraft:speed 600 1)',
            experience: 'Give XP',
            experience_prompt: 'XP amount (e.g. 100)',
            title_action: 'Send Title',
            title_prompt: 'Title text',
            message: 'Private Message',
            message_prompt: 'Message content',
            spawnpoint: 'Reset Spawnpoint',
            whitelist_add: 'Add to Whitelist',
            op_give: 'Give OP',
            ban_ip: 'Ban IP',
            ban_reason_prompt: 'Ban reason (optional)',
            ban_reason_placeholder: 'Enter ban reason...',
            effect_type: 'Effect Type',
            effect_duration: 'Duration',
            seconds: 's',
            effect_level: 'Effect Level',
            level_unit: 'Lvl',
            hide_particles: 'Hide Particle Effects',
            clear_effects: 'Clear All Effects',
            inventory: 'View Inventory',
            total_items: 'Total Items',
            unique_items: 'Unique Items',
            enchanted: 'Enchanted',
            op_remove: 'Remove OP',
            armor: 'Armor',
            offhand: 'Offhand',
            hotbar: 'Hotbar'
        },
        easyauth: {
            users: 'Users',
            registered: 'Registered',
            unregistered: 'Unregistered',
            password: 'Password',
            unregister: 'Unregister',
            confirm_unregister: 'Unregister player',
            config: 'Config',
            new_password_prompt: 'Enter new password (>4 chars):',
            new_password_placeholder: 'New Password...',
            config_groups: {
                login_session: 'Login Session',
                global_password: 'Global Password',
                others: 'Others',
                restrictions: 'Restrictions',
                rules: 'Rules',
                storage: 'Storage',
                technical: 'Technical'
            },
            config_labels: {
                premium_auto_login: 'Premium Auto Login',
                offline_by_default: 'Offline by Default',
                session_timeout: 'Session Timeout (s)',
                max_login_tries: 'Max Login Tries',
                kick_timeout: 'Kick Timeout (s)',
                enable_global_password: 'Enable Global Password',
                single_use_global_password: 'Single Use Global Password',
                hide_player_coords: 'Hide Player Coords',
                debug: 'Debug Mode',
                allow_chat: 'Allow Chat',
                allow_commands: 'Allow Commands',
                allow_movement: 'Allow Movement',
                allow_block_interaction: 'Allow Block Interaction',
                allow_block_breaking: 'Allow Block Breaking',
                hide_inventory: 'Hide Inventory',
                player_invulnerable: 'Player Invulnerable',
                min_password_length: 'Min Password Length',
                max_password_length: 'Max Password Length',
                username_regexp: 'Username Regex',
                enable_server_side_translation: 'Enable Server-side Translation',
                database_type: 'Database Type',
                sqlite_path: 'SQLite Path',
                sqlite_table: 'SQLite Table',
                mysql_host: 'MySQL Host',
                mysql_user: 'MySQL User',
                mysql_password: 'MySQL Password',
                mysql_database: 'MySQL Database',
                mysql_table: 'MySQL Table',
                mongodb_connection_string: 'MongoDB Connection String',
                mongodb_database: 'MongoDB Database',
                use_simpleauth_db: 'Use SimpleAuth DB',
                global_password_hash: 'Global Password Hash'
            }
        },
        voice: {
            title: 'Simple Voice Chat',
            general: 'General & Network',
            network: 'Network & Connection',
            port: 'Port',
            port_desc: 'Voice audio communication UDP port (Default 24454)',
            host: 'Voice Host',
            host_desc: 'Public IP or domain with port for clients, e.g. mc.example.com:24454',
            bind_address: 'Bind Address',
            bind_address_desc: 'Local IP address to bind to. Leave blank or use * for wildcard',
            password: 'Password (Legacy)',
            saved: 'Config Saved (Restart Required)',
            restart_required: 'Restart server to apply configuration changes.',
            quality_distance: 'Voice Distance & Quality',
            max_distance: 'Max Voice Distance',
            max_distance_desc: 'Maximum distance in blocks voice can be heard (Default 48.0)',
            whisper_distance: 'Whisper Distance',
            whisper_distance_desc: 'Maximum distance whisper voice can be heard (Default 24.0)',
            broadcast_range: 'Broadcast Range',
            broadcast_range_desc: 'Broadcast range in blocks (-1 means same as max distance)',
            codec: 'Opus Codec',
            codec_desc: 'Opus codec profile: VOIP, AUDIO, or RESTRICTED_LOWDELAY',
            mtu_size: 'MTU Size',
            mtu_size_desc: 'Maximum size of audio packets in bytes (Default 1024)',
            crouch_distance: 'Crouch Distance (Legacy)',
            bitrate: 'Bitrate (Legacy)',
            max_packet_weight: 'Max Packet Weight (Legacy)',
            groups_permissions: 'Groups & Permissions',
            enable_groups: 'Enable Groups',
            enable_groups_desc: 'Allow players to create and join private voice groups',
            allow_recording: 'Allow Recording',
            allow_recording_desc: 'Allow players to record voice chat audio',
            spectator_interaction: 'Spectator Interaction',
            spectator_interaction_desc: 'Allow spectator players to talk with living players',
            spectator_player_possession: 'Spectator Possession',
            spectator_player_possession_desc: 'Allow spectators to talk with possessed players',
            security_performance: 'Security & Performance',
            force_voice_chat: 'Force Voice Chat Mod',
            force_voice_chat_desc: 'Kick players who do not have the voice chat mod installed',
            login_timeout: 'Login Timeout (ms)',
            login_timeout_desc: 'Wait time before kicking players without mod (Default 10000)',
            tcp_rate_limit: 'TCP Rate Limit',
            tcp_rate_limit_desc: 'Max voice chat packets per second (-1 to disable, default 16)',
            keep_alive: 'Keep Alive Interval (ms)',
            keep_alive_desc: 'Frequency of keep-alive packets in ms (Default 1000)',
            allow_pings: 'Allow Pings',
            allow_pings_desc: 'Allow voice chat server to reply to external pings',
            use_natives: 'Use Native Libraries',
            use_natives_desc: 'Load native libraries on dedicated servers',
            threaded_server_support: 'Threaded Server Support',
            threaded_server_support_desc: 'Support for multi-threaded servers such as Folia',
            template_notice: 'Default configuration template loaded. Click Save to write the file.',
            open_anyway: 'Force Open Config'
        },
        properties: {
            title: 'Server Settings (server.properties)',
            server_icon: 'Server Icon',
            reset_icon_confirm: 'Reset Icon (Delete)?',
            icon_tips: 'Supports any image (will be auto-cropped and converted to 64x64 PNG)',
            icon_auto_converted: 'Image automatically cropped and converted to 64x64 PNG format',
            port_conflict_title: 'Port Conflict',
            port_conflict_msg: 'Port [{port}] is already in use, the server cannot start. Do you want to automatically reassign it to an available port and start?',
            port_reassign_success: 'Port successfully reassigned to {port}, starting the server...',
            filter: 'Filter settings...',
            show_advanced: 'Show Advanced Properties',
            restart_tips: 'Restart server to apply changes.',
            key: 'Key',
            value: 'Value',
            groups: {
                general: 'General',
                gameplay: 'Gameplay',
                world: 'World Generation',
                spawning: 'Spawning',
                network: 'Performance & Network',
                security: 'Security & Remote',
                resource_pack: 'Resource Pack',
                permissions: 'Permissions',
                management: 'Management Server',
                other: 'Other Settings'
            },
            labels: {
                motd: 'MOTD',
                'server-port': 'Port',
                'max-players': 'Max Players',
                'online-mode': 'Online Mode',
                'white-list': 'Whitelist',
                'enable-rcon': 'Enable RCON',
                'rcon.password': 'RCON Password',
                'rcon.port': 'RCON Port',
                gamemode: 'Gamemode',
                'force-gamemode': 'Force Gamemode',
                difficulty: 'Difficulty',
                hardcore: 'Hardcore',
                pvp: 'PVP',
                'allow-flight': 'Allow Flight',
                'level-seed': 'Seed',
                'level-type': 'Level Type',
                'level-name': 'Level Name',
                'generate-structures': 'Generate Structures',
                'generator-settings': 'Generator Settings',
                'allow-nether': 'Allow Nether',
                'spawn-monsters': 'Spawn Monsters',
                'spawn-animals': 'Spawn Animals',
                'spawn-npcs': 'Spawn NPCs',
                'spawn-protection': 'Spawn Protection',
                'view-distance': 'View Distance',
                'simulation-distance': 'Simulation Distance',
                'max-tick-time': 'Max Tick Time',
                'rate-limit': 'Rate Limit',
                'enable-query': 'Enable Query',
                'query.port': 'Query Port',
                'enable-status': 'Enable Status',
                'enforce-secure-profile': 'Enforce Secure Profile',
                'enforce-whitelist': 'Enforce Whitelist',
                'entity-broadcast-range-percentage': 'Entity Broadcast Range',
                'function-permission-level': 'Function Permission Level',
                'op-permission-level': 'OP Permission Level',
                'player-idle-timeout': 'Player Idle Timeout',
                'prevent-proxy-connections': 'Prevent Proxy Connections',
                'network-compression-threshold': 'Network Compression Threshold',
                'max-world-size': 'Max World Size',
                'sync-chunk-writes': 'Sync Chunk Writes',
                'use-native-transport': 'Use Native Transport',
                'enable-jmx-monitoring': 'Enable JMX Monitoring',
                'broadcast-console-to-ops': 'Broadcast Console to OPs',
                'broadcast-rcon-to-ops': 'Broadcast RCON to OPs',
                'require-resource-pack': 'Require Resource Pack',
                'resource-pack': 'Resource Pack URL',
                'resource-pack-sha1': 'Resource Pack SHA1',
                'resource-pack-prompt': 'Resource Pack Prompt',
                'resource-pack-id': 'Resource Pack ID',
                'server-ip': 'Server IP',
                'hide-online-players': 'Hide Online Players',
                'initial-enabled-packs': 'Initial Enabled Packs',
                'initial-disabled-packs': 'Initial Disabled Packs',
                'max-chained-neighbor-updates': 'Max Chained Neighbor Updates',
                'log-ips': 'Log IPs',
                'region-file-compression': 'Region File Compression',
                'pause-when-empty-seconds': 'Pause When Empty (seconds)',
                'accepts-transfers': 'Accepts Transfers',
                'enable-code-of-conduct': 'Enable Code of Conduct',
                'bug-report-link': 'Bug Report Link',
                'text-filtering-config': 'Text Filtering Config',
                'text-filtering-version': 'Text Filtering Version',
                'management-server-enabled': 'Enable Management Server',
                'management-server-host': 'Management Server Host',
                'management-server-port': 'Management Server Port',
                'management-server-secret': 'Management Server Secret',
                'management-server-allowed-origins': 'Management Server Allowed Origins',
                'management-server-tls-enabled': 'Enable TLS',
                'management-server-tls-keystore': 'TLS Keystore',
                'management-server-tls-keystore-password': 'TLS Keystore Password',
                'status-heartbeat-interval': 'Status Heartbeat Interval',
                'enable-command-block': 'Enable Command Block'
            },
            fabric_version: 'Fabric Core Version',
            loader_version: 'Loader Core Version',
            loader_type: 'Loader Type',
            current_mc: 'Current MC Version',
            current_loader: 'Current Loader Version',
            change_version: 'Change Version',
            target_mc: 'Target MC Version',
            target_loader: 'Target Loader Version',
            choose_mc: 'Select MC Version',
            choose_loader: 'Select Loader Version',
            apply: 'Apply',
            stop_server_first: 'Please stop the server before changing version',
            version_change_tips: 'Changing the core version will not affect worlds or configs, but may be incompatible with some mods. Check mod compatibility after changing.',
            fetch_versions_fail: 'Failed to fetch version list',
            change_version_confirm: 'Are you sure you want to change the loader core to MC {mc} / Loader {loader}?',
            version_changing: 'Downloading and changing loader core...',
            version_change_success: 'Loader core version changed successfully',
            version_change_timeout: 'Version change timed out, please check server logs for status'
        },
        panel_settings: {
            title: 'Panel Settings',
            basic: 'Basic Settings',
            server: 'Server Configuration',
            security: 'Security Settings',
            integrations: 'Integrations',
            advanced: 'Advanced Settings',
            port: 'Panel Port',
            port_desc: 'Requires panel restart',
            host: 'Listening Address (Host)',
            host_desc: 'Network interface the panel binds to. 0.0.0.0 allows all interfaces/remote access, 127.0.0.1 limits to localhost',
            host_all: '0.0.0.0 (All interfaces / Remote allowed)',
            host_local: '127.0.0.1 (Localhost only)',
            lan_addresses: 'Detected LAN Addresses',
            default_lang: 'Default Language',
            theme: 'Theme Mode',
            theme_light: 'Light',
            theme_dark: 'Dark',
            theme_auto: 'Auto',
            jar_name: 'Server Startup File',
            jar_name_desc: 'Select the JAR file to start the server',
            java_args: 'Java Arguments',
            java_args_desc: 'One argument per line, e.g. -Xms1G',
            secret: '2FA Secret',
            secret_masked: 'Secret is hidden',
            session_timeout: 'Session Timeout (days)',
            max_log_history: 'Max Log History',
            monitor_interval: 'Monitor Interval (ms)',
            reset_2fa: 'Reset 2FA',
            restart_panel: 'Restart Panel',
            restart_now: 'Restart Now',
            restart_later: 'Later',
            save_success: 'Configuration Saved',
            restart_required: 'Restart required to apply changes',
            restart_confirm: 'Configuration saved. Restart panel to apply changes?',
            reset_2fa_confirm: 'Reset 2FA? This will invalidate all logged-in devices. You will need to scan the QR code again.',
            reset_2fa_setup: 'Setup New 2FA',
            restart_warning: 'Restarting the panel will disconnect all connections. Continue?',
            reset_2fa_success: '2FA Reset',
            reset_2fa_instruction: 'Please scan the new QR code with your authenticator',
            restarting: 'Panel is restarting...',
            validation_error: 'Validation Failed',
            reinstall: 'Reinstall MC Server',
            reinstall_confirm: 'This will delete ALL files of the current MC server (including worlds and configs). Are you sure?',
            reinstall_success: 'Reset successful, redirecting to setup wizard',
            ai_settings: 'AI Settings',
            ai_endpoint: 'API Endpoint',
            ai_key: 'API Key',
            ai_model: 'Model Name',
            ai_endpoint_desc: 'OpenAI-compatible URL, e.g., https://api.openai.com/v1',
            ai_key_desc: 'Leave blank if no key required',
            ai_model_placeholder: 'e.g., gpt-3.5-turbo',
            ai_test: 'Test Connection',
            ai_test_success: 'Connection Success',
            ai_test_fail: 'Connection Failed',
            webhook_settings: 'Event Webhook',
            webhook_urls: 'Webhook Receivers (one URL per line)',
            webhook_desc: 'The panel will send POST event notifications to these URLs when the server starts, stops, crashes, or players join/leave.',
            console_info_position: 'Console Info Position',
            pos_top: 'Top',
            pos_sidebar: 'Sidebar',
            pos_hide: 'Hide',
            github_proxy: 'GitHub Proxy',
            github_proxy_desc: 'Proxy for faster updates and Java downloads',
            backup_maintenance: 'Backup & Maintenance',
            create_global_backup: 'Create Global Backup',
            backup_include: 'Include Content',
            backup_configs: 'Panel Configs',
            backup_java: 'Java Environments',
            backup_instances: 'Server Instances',
            restore_global_confirm: 'Are you sure you want to restore backup {name}? This will overwrite all current configs and instances. The panel will restart after completion.',
            restoring: 'Applying backup, panel will restart...',
            import_backup: 'Import Backup',
            no_instances: 'No instances found',
            no_java: 'No Java downloaded from panel',
            appearance: 'Appearance',
            custom_logo: 'Custom Logo',
            logo_desc: 'Square image recommended. Replaces default icon in login page and instance detail page',
            background_image: 'Background Image',
            background_desc: 'Set a custom background image for the panel',
            sidebar_opacity: 'Sidebar Opacity',
            content_opacity: 'Content Opacity',
            card_opacity: 'Card Opacity',
            login_opacity: 'Login Page Opacity',
            instance_opacity: 'Instance List Opacity',
            appearance_preview: 'Preview',
            tab_general: 'General',
            tab_login: 'Login Page',
            tab_instance_list: 'Instance List',
            tab_instance_detail: 'Instance Detail',
            tab_other: 'Other Pages',
            other_interfaces_desc: 'Java Manager, Plugin Loader, FRP Manager and other extension pages inherit general appearance settings. Adjust here if needed.',
            upload: 'Upload',
            upload_success: 'Upload successful',
            account_mgmt: 'Account Management',
            admin_user: 'Admin Username',
            current_pass: 'Current Password (Required)',
            new_pass: 'New Password (leave blank to keep)',
            confirm_new_pass: 'Confirm New Password',
            disable_2fa: 'Disable 2FA',
            enable_2fa: 'Enable 2FA',
            disable_2fa_confirm: 'Are you sure you want to disable 2FA? This lowers account security.',
            account_update_success: 'Admin account updated successfully',
            account_update_fail: 'Failed to update account credentials'
        },

        panel_backup: {
            title: 'Panel Backup',
            description: 'Manage full panel data backups, configs, Java environments and instance archives',
            create_btn: 'Create Global Backup',
            import_btn: 'Import Backup',
            restore_btn: 'Restore Backup',
            restore_title: 'System Restore',
            backup_content: 'Included Content',
            opt_config: 'Configs',
            prompt_note: 'Backup Note (Optional)',
            backup_include: 'Select Backup Content',
            backup_configs: 'System & Panel Configs (data/*.json)',
            backup_java: 'Installed Java Runtimes',
            backup_instances: 'Minecraft Instances',
            no_instances: 'No instances found',
            no_java: 'No panel-managed Java installed',
            restore_confirm: 'Are you sure you want to restore the system from {name}?',
            ask_restore_now: 'Backup {name} imported successfully! Do you want to restore it now?',
            restoring: 'Restoring backup safely, please wait...',
            confirm_delete_msg: 'Are you sure you want to delete backup archive {name}? This cannot be undone.',
            uploading: 'Uploading backup archive...',
            import_success: 'Backup imported successfully!'
        },

        plugins: {
            title: 'Plugin Manager',
            description: 'Manage and configure panel plugins to extend functionality',
            official: 'Official Plugins',
            official_badge: 'Official',
            third_party: 'Third-Party Plugins',
            running: 'Running',
            stopped: 'Stopped',
            error: 'Load Error',
            install: 'Install Plugin',
            upload_desc: 'Please upload a plugin package in .zip format. It will be uploaded to a temporary directory and installed automatically.',
            update: 'Update Plugin',
            update_title: 'Update Plugin: {name}',
            install_confirm_title: 'Confirm Plugin Installation',
            update_badge: 'Update',
            new_badge: 'New Plugin',
            update_comparison: 'Version Comparison',
            current_version: 'Current',
            new_version: 'Target',
            downgrade_warning: 'Warning: The target version is lower than the current installed version.',
            install_confirm_hint: 'Please review the plugin information above and click confirm to proceed.',
            confirm_update: 'Confirm Update',
            confirm_install: 'Confirm Installation',
            upload_analyze: 'Upload & Analyze',
            no_plugins: 'No Plugins',
            no_plugins_hint: 'Install plugins to extend panel functionality',
            enabled_success: '{name} enabled',
            disabled_success: '{name} disabled',
            uninstall_title: 'Uninstall Plugin',
            uninstall_confirm: 'Are you sure you want to uninstall {name}?',
            uninstall_warning_hint: 'Uninstalling this plugin will permanently delete all its files and data. This action cannot be undone.',
            type_to_confirm_hint: 'Please type the plugin name {name} to confirm deletion:',
            type_to_confirm: 'Type plugin name to confirm',
            confirmation_mismatch: 'Name does not match',
            uninstalled_success: '{name} uninstalled',
            err_invalid_zip: 'Plugin file is not a valid ZIP archive',
            err_manifest_not_found: 'Plugin manifest (plugin.json) not found',
            err_missing_id: 'Plugin manifest missing "id" field',
            err_install_info: 'Missing install info',
            install_title: 'Install Plugin',
            install_warning_title: 'Security Warning',
            install_warning_text: 'Third-party plugins may contain malicious code. Only install from trusted sources. Plugins have high privileges including system commands and file system access.',
            install_path_label: 'Plugin Path',
            install_path_placeholder: 'Enter absolute path to plugin directory or .zip file',
            install_path_hint: 'Plugin directory must contain a plugin.json manifest',
            install_file_label: 'Upload Plugin Package',
            install_file_hint: 'Supports .zip format, must contain plugin.json',
            install_button: 'Install',
            no_install_source: 'Please provide a plugin path or upload a package',
            install_success: 'Plugin installed successfully',
            update_success: 'Plugin updated successfully',
            update_button: 'Update',
            guide_button: 'Dev Guide',
            market_button: 'Plugin Store',
            guide_title: 'Plugin Dev Guide',
            guide_desc: 'Learn how to build custom plugins for Cloud Language Panel',
            guide_step1_title: 'Quick Start',
            guide_step1_desc: 'Start by creating a folder with a plugin.json file.',
            guide_step2_title: 'Coding',
            guide_step2_desc: 'Write backend logic and frontend UI using JavaScript.',
            guide_step3_title: 'Publishing',
            guide_step3_desc: 'Compress your folder as a .zip to install it in the panel.',
            guide_structure_title: 'Standard Structure',
            guide_structure_desc: 'A standard plugin package should contain:',
            guide_file_json: 'Manifest (Required)',
            guide_file_main: 'Backend Entry',
            guide_file_public: 'Static Assets',
            guide_file_frontend: 'Frontend Entry',
            guide_toc: 'Table of Contents',
            guide_export: 'Export to MD',
            export_md: 'Export as Markdown',
            intro: 'Getting Started',
            manifest: 'plugin.json Reference',
            dependencies: 'NPM Dependencies',
            backend: 'Backend & API',
            frontend: 'Frontend Dev',
            'vue-options': 'Vue Options & Components',
            i18n: 'Internationalization (i18n)',
            socket: 'Socket.IO',
            lifecycle: 'Lifecycle',
            'panel-api': 'Panel API Endpoints',
            css: 'CSS Style Reference',
            tips: 'Dev Tips',
            tasks: 'Persistent & Scheduled Tasks',
            permissions: 'Permission System',
            'plugin-settings': 'Plugin Settings',
            storage: 'KV Storage',
            events: 'Inter-plugin Communication',
            status: 'Status Monitoring',
            'plugin-deps': 'Plugin Dependencies',
            'api-version': 'API Versioning',
            disclaimer_title: 'Plugin Security Disclaimer',
            disclaimer_line1: 'The plugin system has high privileges, including but not limited to executing system commands, viewing system status, running external programs, and operating the file system.',
            disclaimer_line2: 'Before installing third-party plugins, please ensure the source is trustworthy. The panel is not responsible for any data loss, system damage, or security issues caused by third-party plugins.',
            disclaimer_line3: 'By using this plugin system, you acknowledge and accept the above risks. All consequences arising from plugin usage are borne by the user.',
            disclaimer_confirm: 'I have read and understand the above risk warnings and disclaimer',
            disclaimer_agree: 'Confirm and Continue',
            settings: 'Settings',
            settings_saved: 'Settings saved',
            no_settings: 'This plugin has no registered settings',
            permissions_title: 'Permissions',
            plugin_dependencies: 'Plugin Dependencies',
        },

        setup: {
            welcome: 'Welcome to MC Panel',
            desc: 'Your server directory appears to be empty. Follow the steps to install a new Minecraft server.',
            select_loader_type: 'Select Loader Type',
            select_mc: 'Select Game Version',
            select_loader: 'Select Loader Version',
            choose: 'Choose...',
            install: 'Install Server',
            installing: 'Installing...',
            install_start_toast: 'Installation started. Please wait...',
            install_success_toast: 'Installation successful! Refreshing...',
            install_fail_toast: 'Installation failed',
            install_timeout_toast: 'Installation timed out (check logs)',
            fetch_fail_mc: 'Failed to fetch MC versions',
            fetch_fail_loader: 'Failed to fetch Loader versions',
            loading_versions: 'Loading versions...',
            restore_from_backup: 'Restore from Backup',
            restoring_uploading: 'Uploading backup...',
            restoring_applying: 'Applying backup, panel will restart...'
        },
        java: {
            title: 'Java Management',
            installed: 'Installed',
            online_install: 'Online Install',
            detect_system: 'Detect System Java',
            add_local: 'Add Local Java',
            local_path_placeholder: 'Enter Java executable path, e.g. /usr/bin/java',
            local_path_desc: 'Specify the path to a locally installed Java executable',
            no_installed: 'No Java versions installed',
            no_available: 'Unable to fetch available versions, check network',
            fetching: 'Fetching available versions...',
            fetch_fail: 'Failed to fetch available versions',
            install: 'Install',
            already_installed: 'Installed',
            confirm_remove: 'Are you sure you want to remove {name}? This cannot be undone.',
            system_not_found: 'System Java not detected',
            system_found: 'System Java detected',
            add_to_list: 'Add to managed list?',
            task_running: 'An installation task is already running'
        },
        instance_manager: {
            title: 'Instance Manager',
            create_btn: 'Create New Instance',
            manage_java: 'Manage Java',
            search_placeholder: 'Search instances...',
            state_starting: 'Starting',
            state_running: 'Running',
            state_stopping: 'Stopping',
            state_stopped: 'Stopped',
            online_players: ' players online',
            select_btn: 'Enter Panel',
            settings_btn: 'Instance Settings',
            delete_btn: 'Delete Instance',
            create_modal_title: 'Create New Instance',
            edit_modal_title: 'Edit Instance Settings',
            name_label: 'Instance Name',
            name_placeholder: 'e.g. Redstone Survival',
            jar_placeholder: 'server.jar (Optional)',
            java_path_label: 'Java Path',
            java_args_label: 'JVM Arguments',
            java_args_placeholder: '-Xms1G -Xmx4G',
            java_args_recommend_tip: 'Total memory {total}G, recommended maximum allocation is -Xmx{recommend}G (e.g. -Xmx{recommend}G)',
            forge_jar_tip: 'Forge/NeoForge startup jars are auto-generated by the installer. Do not modify unless you know what you are doing.',
            run_sh_mode: 'run.sh mode',
            jar_mode: 'jar mode',
            user_jvm_args_tip: 'JVM arguments will be automatically written to user_jvm_args.txt on startup',
            delete_confirm: 'Are you sure you want to delete instance "{name}"? This will permanently delete all data for this instance and cannot be undone!',
            create_success: 'Instance created successfully',
            update_success: 'Settings Updated',
            delete_success: 'Instance Deleted',
            back_to_list: 'Back to List',
            no_instances: 'No instances, click top right to create',
            action_sent: 'Instance {name}: {action} command sent',
            select_fail: 'Failed to select instance',
            instance_count: '{count} instances'
        },

        frp: {
            title: 'FRP Tunnel',
            install_manage: 'Installation',
            not_installed: 'FRP client not installed',
            install: 'Install',
            install_first: 'Please install FRP client first',
            installing: 'Installing...',
            installed: 'Installed',
            version: 'Version',
            uninstall: 'Uninstall',
            uninstall_confirm: 'Are you sure you want to uninstall FRP client? The binary will be removed but config will be kept.',
            config: 'Configuration',
            config_list: 'Configurations',
            new_config: 'New Config',
            edit_config: 'Edit Config',
            config_name: 'Config Name',
            config_name_placeholder: 'e.g. My Server',
            config_name_required: 'Please enter a config name',
            no_configs: 'No configs yet',
            no_configs_hint: 'Create an FRP config to start tunneling',
            delete_config_confirm: 'Delete config "{name}"? Running processes will be stopped.',
            gui_mode: 'GUI Mode',
            text_mode: 'Text Mode',
            server_addr: 'Server Address',
            server_port: 'Server Port',
            auth_token: 'Auth Token',
            tunnels: 'Tunnels',
            tunnel_name: 'Name',
            tunnel_type: 'Protocol',
            local_ip: 'Local IP',
            local_port: 'Local Port',
            remote_port: 'Remote Port',
            add_tunnel: 'Add Tunnel',
            no_tunnels: 'No tunnels yet, click above to add',
            start: 'Start',
            stop: 'Stop',
            restart: 'Restart',
            running: 'Running',
            stopped: 'Stopped',
            logs: 'Logs',
            save_config: 'Save Config',
            config_saved: 'Configuration saved',
            version_manager: 'Version Manager',
            switch_version: 'Install / Switch',
            no_releases: 'Cannot fetch releases, check network',
            fetch_releases: 'Fetching releases...',
            latest: 'Latest',
            server_addr_desc: 'Public IP or domain of frps server',
            server_port_desc: 'bindPort in frps.toml',
            auth_token_desc: 'Must match server token',
            started_success: 'FRP started',
            stopped_success: 'FRP stopped',
            start_fail: 'Start failed',
            use_github_proxy: 'Use GitHub Proxy',
            proxy_url: 'Proxy URL',
            proxy_url_placeholder: 'e.g. https://ghfast.top/ or https://mirror.ghproxy.com/',
            proxy_tips: 'Recommended if GitHub download is slow or timing out',
            proxy_presets: 'Presets',
            import_config: 'Import Config',
            config_format: 'Config Format',
            format_toml: 'TOML Format (.toml)',
            format_ini: 'INI Format (.ini)',
            import_file: 'Import File',
            import_file_tip: 'Supports importing .ini, .toml, .conf, .txt config files',
            import_clipboard: 'Clipboard',
            import_clipboard_tip: 'Paste tunnel config directly from clipboard',
            clipboard_modal_title: 'Paste Config from Clipboard',
            clipboard_modal_desc: 'Paste your tunnel configuration (from SakuraFrp, ME Frp, OpenFrp, etc.) below:',
            clipboard_modal_confirm: 'Confirm Import',
            import_success: 'Imported successfully (detected as {format} format)',
            import_fail: 'Import failed: Unable to parse valid config',
            clipboard_empty: 'Clipboard is empty',
            clipboard_denied: 'Clipboard access denied, opened paste dialog instead',
            format_switch_tip: 'Configuration will be converted when switching formats',
            user: 'User ID',
            user_desc: 'User ID required by some tunnel platforms (e.g. user field in SakuraFrp)',
            clear_logs: 'Clear Logs',
            logs_cleared: 'Logs cleared',
            copy_logs: 'Copy Logs',
            logs_copied: 'Logs copied to clipboard',
            auto_scroll: 'Auto Scroll',
            back_to_config: 'Back to Config'
        }
    }
};

export const createI18n = (store) => {
    return (key, params = {}) => {
        const keys = key.split('.');
        let value = messages[store.lang];
        for (const k of keys) {
            value = value?.[k]; // Use optional chaining to prevent crash
            if (!value) return key;
        }
        // Simple interpolation {name}
        if (typeof value === 'string') {
            return value.replace(/{(\w+)}/g, (_, k) => params[k] !== undefined ? params[k] : `{${k}}`);
        }
        return value;
    };
};
